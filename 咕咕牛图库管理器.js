...
// const RAW_URL_Repo2 = 'https://raw.githubusercontent.com/GuGuNiu/Miao-Plugin-MBT-2/main' // 保留备用
...
function ExecuteCommand(command, args, options = {}, timeout = 0, onStdErr, onStdOut) {
  return new Promise((resolve, reject) => {
      ...
      if (proc && proc.pid && !killed && !exited && !proc.killed) {
        ...
        try {
          ...
          if (killError.code !== 'ESRCH') {
            logger.error(`${Default_Config.logPrefix} [执行命令] kill ${proc.pid} (或进程组) 失败:`, killError);
          } else {
            logger.warn(`${Default_Config.logPrefix} [执行命令] kill ${proc.pid} 时进程已不存在 (ESRCH)`);
          }
          killed = true;
        }
      }
      ...
      timer = setTimeout(() => {
        ...
        setTimeout(() => {
          if (!exited && !promiseSettled) {
             logger.warn(`${Default_Config.logPrefix} [执行命令] SIGTERM 后进程未退出，发送 SIGKILL...`);
             killProc('SIGKILL');
          }
        }, 3000);
    ...
    proc.on('close', (code, signal) => {
      if (exited || promiseSettled) return;
      exited = true;
      logger.info(`${Default_Config.logPrefix} [执行命令] 进程关闭 [${cmdStr}] Code: ${code}, Signal: ${signal}`);
      clearTimeout(timer);
...
export class MiaoPluginMBT extends plugin {
  ...
  static banMutex = new SimpleAsyncMutex();
  ...
  async ManualRunUpdateTask(e) {
    ...
    this.logger.info(`${this.logPrefix} 用户 ${e.user_id} 手动触发更新任务 (#执行咕咕牛更新) 以测试通知逻辑...`);
    await e.reply(`${this.logPrefix} 正在模拟定时更新流程，请稍候... (将检查是否触发通知主人)`, true);
    let overallHasChanges = false;
    let taskError = null;

    try {
      overallHasChanges = await this.UpdateTuKu(null, true); // 模拟定时任务调用
      this.logger.info(`${this.logPrefix} 模拟定时更新任务的核心逻辑已完成。检测到更新: ${overallHasChanges}`);
    ...
    // 根据结果回复触发者
    if (taskError) {
        await e.reply(`${this.logPrefix} 模拟定时更新执行时遇到错误，无法判断通知状态。`, true);
    } else {
        if (overallHasChanges) {
            await e.reply(`${this.logPrefix} 模拟定时更新检测到变更，已尝试向主人发送通知。 (请主人检查私聊)`, true);
        } else {
            await e.reply(`${this.logPrefix} 模拟定时更新未检测到变更，因此未发送通知。`, true);
        }
    }
    return true;
  }
  ...
  async DownloadTuKu(e) {
    ...
    try {
      ...
      // TODO: if (Repo3UrlConfigured && !Repo3Exists) allDownloaded = false;
      if (allDownloaded) {
      ...
      // --- 串行下载核心仓库 ---
      if (!Repo1Exists) {
        logger.info(`${logPrefix} [核心优先下载] 开始下载核心仓库 (一号)...`);
        try {
          ...
          if (!coreRepoResult.success) {
             logger.error(`${logPrefix} [核心优先下载] 核心仓库下载失败。`);
             finalUserMessage = `『咕咕牛』核心仓库下载失败 (${coreRepoResult.nodeName})。请检查日志或网络后重试。`;
          ...
          logger.info(`${logPrefix} [核心优先下载] 核心仓库下载成功 (${coreRepoResult.nodeName})。`);
        } catch (err) {
          logger.error(`${logPrefix} [核心优先下载] 核心仓库下载过程中发生意外错误:`, err);
          coreRepoResult = { repo: 1, success: false, nodeName: '执行异常', error: err };
          await this.ReportError(e, '下载核心仓库', coreRepoResult.error);
          return true;
        }
      } else {
        logger.info(`${logPrefix} [核心优先下载] 核心仓库已存在，跳过下载。`);
      ...
      if (Repo2UrlConfigured && !Repo2Exists) {
        logger.info(`${logPrefix} [核心优先下载] 添加附属仓库 (二号) 下载任务。`);
        subsidiaryPromises.push(
          MiaoPluginMBT.DownloadRepoWithFallback(
            ...
            .catch(err => {
              logger.error(`${logPrefix} [核心优先下载] 附属仓库 (二号) 下载 Promise 捕获到错误:`, err);
              return { repo: 2, success: false, nodeName: '执行异常', error: err };
            })
        );
      } else if (Repo2UrlConfigured && Repo2Exists) {
         logger.info(`${logPrefix} [核心优先下载] 附属仓库 (二号) 已存在。`);
         subsidiaryResults.push({ repo: 2, success: true, nodeName: '本地', error: null });
      } else {
         logger.info(`${logPrefix} [核心优先下载] 附属仓库 (二号) 未配置。`);
      }

      // TODO: 未来在这里添加三号仓库的处理逻辑

      if (subsidiaryPromises.length > 0) {
          logger.info(`${logPrefix} [核心优先下载] 等待 ${subsidiaryPromises.length} 个附属仓库下载完成...`);
          const settledResults = await Promise.allSettled(subsidiaryPromises);
          logger.info(`${logPrefix} [核心优先下载] 所有附属仓库 Promise 已完成 (settled)。`);
          settledResults.forEach(result => {
              if (result.status === 'fulfilled') {
                  subsidiaryResults.push(result.value);
                  if(result.value.success){
                      logger.info(`${logPrefix} [核心优先下载] 附属仓库 (${result.value.repo}号) 下载成功 (${result.value.nodeName})。`);
                  } else {
                      logger.error(`${logPrefix} [核心优先下载] 附属仓库 (${result.value.repo}号) 下载失败 (${result.value.nodeName})。`);
                      logger.error(`${logPrefix} [核心优先下载] 失败详情:`, result.value.error);
                  }
              } else {
                  logger.error(`${logPrefix} [核心优先下载] 一个附属仓库 Promise rejected:`, result.reason);
              }
          });
      }

      // --- 结果汇报 ---
      let coreStatusLine = `✅ 核心：下载成功 (${coreRepoResult.nodeName})。`;

      let subsidiaryStatusLines = [];
      subsidiaryResults.forEach(res => {
          let statusEmoji = res.success ? '✅' : '⚠️';
          let statusText = '';
          if (res.nodeName === '本地') {
              statusText = '已存在';
          } else if (res.nodeName === '未配置') {
              statusText = '未配置';
          } else {
              statusText = res.success ? `下载成功 (${res.nodeName})` : `下载失败 (${res.nodeName})`;
          }
          let repoDisplayName = `${res.repo}号仓库`;
          if (res.repo === 2) repoDisplayName = '二号仓库';
          // TODO: if (res.repo === 3) repoDisplayName = '三号仓库';

          subsidiaryStatusLines.push(`       - ${repoDisplayName}：${statusText}`);
      });

      let finalReply = `『咕咕牛』\n${coreStatusLine}`;
      if (subsidiaryStatusLines.length > 0) {
          finalReply += `\n✅ 附属：\n${subsidiaryStatusLines.join('\n')}`;
      }

      await e.reply(finalReply).catch(() => {});

      // --- 执行下载后设置 ---
      logger.info(`${logPrefix} [核心优先下载] 开始执行 RunPostDownloadSetup...`);
      await MiaoPluginMBT.RunPostDownloadSetup(e, logger);
      logger.info(`${logPrefix} [核心优先下载] RunPostDownloadSetup 执行完成。`);
      ...
      subsidiaryResults.forEach(async (res) => {
          if (res.success && res.nodeName !== '本地' && res.nodeName !== '未配置') {
              ...
              // TODO: if (res.repo === 3) { repoPath = MiaoPluginMBT.paths.LocalTuKuPath3; repoDisplayName = '三号仓库'; }
              if (repoPath) {
      ...
      logger.info(`${logPrefix} [核心优先下载] 流程结束，耗时 ${duration} 秒。`);
    }
    return true;
  }
  ...
  async UpdateTuKu(e, isScheduled = false) {
    ...
    try {
      ...
      const overallSuccess = Repo1Success && Repo2Success
  ...
  async ManageTuKu(e) {
    ...
    for (const dirPath of pathsToDeleteDirectly) {
      ...
      try {
        ...
        if (!deleted) {
          this.logger.warn(`${this.logPrefix} 删除 ${dirPath} 未完全成功 (可能部分文件残留)`);
        }
      } catch (err) {
    ...
    for (const dirPath of targetPluginDirs) {
      try {
        ...
        cleanSuccess = false;
        if (!firstError) firstError = { operation: `清理目标插件目录 ${path.basename(dirPath)}`, error: err };
      }
    }

    this.logger.info(`${this.logPrefix} 重置内存状态...`);
    await MiaoPluginMBT.configMutex.acquire();
    await MiaoPluginMBT.banMutex.acquire();
    try {
        ...
        MiaoPluginMBT.banMutex.release();
  ...
  async ManageTuKuOption(e) {
    ...
    try {
        ...
        if (currentStatus === enable) {
          ...
          try {
            // logger.debug(`${logPrefix} [启用禁用][诊断] 调用 ShowSettingsPanel (状态未变)...`);
            await this.ShowSettingsPanel(e, `图库已经是「${action}」状态了。`);
            // logger.debug(`${logPrefix} [启用禁用][诊断] ShowSettingsPanel (状态未变) 调用成功。`);
          } catch (panelError) {
        ...
        await this.ReportError(e, `${action}咕咕牛`, configError, '处理配置时出错');
        MiaoPluginMBT.configMutex.release();
    ...
    if (configChanged) {
        try {
          ...
          if (enable) {
            // logger.debug(`${logPrefix} [启用禁用][诊断] 调用 SyncCharacterFolders...`);
            await MiaoPluginMBT.SyncCharacterFolders(logger);
            // logger.debug(`${logPrefix} [启用禁用][诊断] SyncCharacterFolders 调用完成。`);
            // logger.debug(`${logPrefix} [启用禁用][诊断] 调用 GenerateAndApplyBanList...`);
            await MiaoPluginMBT.GenerateAndApplyBanList(MiaoPluginMBT._imgDataCache, logger);
            // logger.debug(`${logPrefix} [启用禁用][诊断] GenerateAndApplyBanList 调用完成。`);
          } else {
            // logger.debug(`${logPrefix} [启用禁用][诊断] 调用 CleanTargetCharacterDirs (Miao)...`);
            await MiaoPluginMBT.CleanTargetCharacterDirs(MiaoPluginMBT.paths.target.miaoChar, logger);
            // logger.debug(`${logPrefix} [启用禁用][诊断] CleanTargetCharacterDirs (Miao) 调用完成。`);
            // logger.debug(`${logPrefix} [启用禁用][诊断] 调用 CleanTargetCharacterDirs (ZZZ)...`);
            await MiaoPluginMBT.CleanTargetCharacterDirs(MiaoPluginMBT.paths.target.zzzChar, logger);
            // logger.debug(`${logPrefix} [启用禁用][诊断] CleanTargetCharacterDirs (ZZZ) 调用完成。`);
            // logger.debug(`${logPrefix} [启用禁用][诊断] 调用 CleanTargetCharacterDirs (Waves)...`);
            await MiaoPluginMBT.CleanTargetCharacterDirs(MiaoPluginMBT.paths.target.wavesChar, logger);
            // logger.debug(`${logPrefix} [启用禁用][诊断] CleanTargetCharacterDirs (Waves) 调用完成。`);
          }
    ...
    try {
      ...
      // logger.debug(`${logPrefix} [启用禁用][诊断] 尝试显示最终面板...`);
      await this.ShowSettingsPanel(e, extraPanelMsg.trim());
      panelSent = true;
      // logger.debug(`${logPrefix} [启用禁用][诊断] 最终面板显示成功。`);
    } catch (panelError) {
      logger.error(`${logPrefix} [启用禁用] 显示设置面板失败，将发送文本回退:`, panelError);
      panelSent = false;
      let finalUserMessage = `${logPrefix} 图库已设为「${action}」。`;
    ...
    // logger.debug(`${logPrefix} [启用禁用][诊断] 函数处理结束。`);
    return true;
  }
  ...
  async PluginInfo(e) {
    ...
    if (logMessages.length > 0) {
      try {
        const forwardMsg = await common.makeForwardMsg(e, logMessages, '『咕咕牛🐂』仓库日志')
        if (forwardMsg) await e.reply(forwardMsg)
  ...
  async ManageUserBans(e) {
    ...
    if (msg === '#ban列表' || msg === '#咕咕牛封禁列表') {
      ...
      const purifiedBansData = []; // 存放 PFL 或开关屏蔽的图片信息 { fileNameNoExt, thumbnailPath, reasons: [] }
      const userBansData = []; // 存放手动封禁的图片信息 { fileNameNoExt, thumbnailPath }
      const pluginVersion = MiaoPluginMBT.GetVersionStatic();
      ...
      await Promise.all(
        sortedActiveBans.map(async relativePath => {
          ...
          if (MiaoPluginMBT._userBanSet.has(relativePath)) {
            ...
            const reasons = []; // 存储屏蔽原因的数组

            // 检查 PFL
            if (imgData && currentPFL > Purify_Level.NONE && MiaoPluginMBT.CheckIfPurifiedByLevel(imgData, currentPFL)) {
                reasons.push('净化'); // 直接使用 "净化"
            }
            ...
            // 如果没有找到任何明确原因（理论上不应发生），添加未知标记
            if (reasons.length === 0) {
                reasons.push('未知');
                logger.warn(`${logPrefix} [封禁列表] 图片 ${relativePath} 在生效列表但未找到明确屏蔽原因?`);
            }

            purifiedBansData.push({ fileNameNoExt, thumbnailPath, reasons }); // 传递原因数组
          }
        })
      );
        ...
        try {
          const renderDataManual = {
           ...
           if (img) {
             ...
             if (purifiedBansData.length > 0) { await common.sleep(1000); }
          ...
          if (tempHtmlFilePathManual && fs.existsSync(tempHtmlFilePathManual)) { try { await fsPromises.unlink(tempHtmlFilePathManual); } catch (unlinkErr) {} }
          if (tempImgFilePathManual && fs.existsSync(tempImgFilePathManual)) { try { await fsPromises.unlink(tempImgFilePathManual); } catch (unlinkErr) {} }
        }
      } else {
      ...
      if (purifiedBansData.length > 0) {
        logger.info(`${logPrefix} [封禁列表] 检测到 ${purifiedBansData.length} 项自动屏蔽项，开始分批处理...`);
        ...
        for (let batchNum = 1; batchNum <= totalBatchesPurified; batchNum++) {
          ...
          logger.info(`${logPrefix} [封禁列表] 准备生成自动屏蔽列表第 ${batchNum}/${totalBatchesPurified} 批 (${currentBatchData.length} 项)...`);
          ...
          try {
            ...
            await fsPromises.mkdir(MiaoPluginMBT.paths.tempHtmlPath, { recursive: true });
            await fsPromises.mkdir(MiaoPluginMBT.paths.tempImgPath, { recursive: true });
            tempHtmlFilePathPurified = path.join(MiaoPluginMBT.paths.tempHtmlPath, `banlist-auto-${timestampPurified}.html`);
            tempImgFilePathPurified = path.join(MiaoPluginMBT.paths.tempImgPath, `banlist-auto-${timestampPurified}.png`);
            await fsPromises.copyFile(sourceHtmlPath, tempHtmlFilePathPurified);
            const imgBatch = await puppeteer.screenshot(`guguniu-banlist-auto-batch${batchNum}`, {
             ...
             if (imgBatch) { forwardListPurified.push(imgBatch); logger.info(`${logPrefix} [封禁列表] 自动屏蔽列表第 ${batchNum}/${totalBatchesPurified} 批图片生成成功。`); }
            ...
            logger.error(`${logPrefix} [封禁列表] 生成自动屏蔽列表第 ${batchNum}/${totalBatchesPurified} 批截图时出错:`, renderBatchError);
            forwardListPurified.push(`[❌ 第 ${batchNum}/${totalBatchesPurified} 批处理出错]`);
            await this.ReportError(e, `生成自动屏蔽列表 (批次 ${batchNum})`, renderBatchError);
          } finally {
            if (tempHtmlFilePathPurified && fs.existsSync(tempHtmlFilePathPurified)) { try { await fsPromises.unlink(tempHtmlFilePathPurified); } catch (unlinkErr) {} }
            if (tempImgFilePathPurified && fs.existsSync(tempImgFilePathPurified)) { try { await fsPromises.unlink(tempImgFilePathPurified); } catch (unlinkErr) {} }
          }
        }

        if (forwardListPurified.length > 1) {
          try {
            const forwardMsgPurified = await common.makeForwardMsg(e, forwardListPurified, '自动屏蔽列表详情');
            if (forwardMsgPurified) { await e.reply(forwardMsgPurified); logger.info(`${logPrefix} [封禁列表] 合并的自动屏蔽列表消息已发送。`); }
            else { logger.error(`${logPrefix} [封禁列表] 创建自动屏蔽列表合并消息失败 (makeForwardMsg 返回空)。`); await e.reply('生成合并的自动屏蔽列表消息失败 (内部错误)。', true); }
          } catch (sendForwardError) {
            logger.error(`${logPrefix} [封禁列表] 发送自动屏蔽列表合并消息失败:`, sendForwardError);
            await e.reply('发送合并的自动屏蔽列表消息失败，请查看日志。', true);
          }
        } else {
           logger.warn(`${logPrefix} [封禁列表] 自动屏蔽列表处理后为空，未发送合并消息。`);
           if (!manualSent) { await e.reply('当前没有手动封禁，也没有被自动规则屏蔽的图片。', true); }
        }

      } else {
          logger.info(`${logPrefix} [封禁列表] 没有被 PFL 或全局开关屏蔽的图片。`);
          if (!manualSent) { await e.reply('当前没有手动封禁，也没有被自动规则屏蔽的图片。', true); }
      }

      if (userBansData.length > 0 && !manualSent && purifiedBansData.length === 0) {
    ...
    if (addMatch || delMatch) {
       ...
       if (!imageData || !imageData.path) {
           ...
           if (!roleExistsInData) { hint = `(图库里好像没有 '${standardMainName}' 这个角色哦)`; }
           else { hint = `(找到了角色 '${standardMainName}'，但是没有找到编号 ${imageNumber} 的图片)`; }
  ...
  async PerformBanOperation(e, isAdding, targetRelativePath, targetFileName, actionVerb) {
    ...
    // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 进入 PerformBanOperation: ${targetFileName}`);
    ...
    try {
        ...
        // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] PFL 检查通过 (等级 ${currentPFL})。`);

        const isCurrentlyUserBanned = MiaoPluginMBT._userBanSet.has(targetRelativePath);
        // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 当前手动封禁状态: ${isCurrentlyUserBanned}`);
        ...
        if (isAdding) {
            if (isCurrentlyUserBanned) {
                ...
                await MiaoPluginMBT.banMutex.acquire();
                // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 获取封禁锁 (添加)。`);
                try {
                    MiaoPluginMBT._userBanSet.add(targetRelativePath);
                    configChanged = true;
                    logger.info(`${logPrefix} [${actionVerb}] 添加到内存封禁列表: ${targetRelativePath}`);
                    replyMsg = `${targetFileName} 🚫 好了，已经手动封禁了。`;
                    saved = await MiaoPluginMBT.SaveUserBans(logger);
                    if (!saved) {
                        logger.error(`${logPrefix} [${actionVerb}] 保存用户封禁列表失败！`);
                    ...
                    MiaoPluginMBT.banMutex.release();
                    // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 释放封禁锁 (添加)。`);
                }
            }
        } else {
            if (!isCurrentlyUserBanned) {
                ...
                await MiaoPluginMBT.banMutex.acquire();
                // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 获取封禁锁 (解禁)。`);
                try {
                    MiaoPluginMBT._userBanSet.delete(targetRelativePath);
                    configChanged = true;
                    needsRestore = true;
                    logger.info(`${logPrefix} [${actionVerb}] 从内存封禁列表移除: ${targetRelativePath}`);
                    ...
                    MiaoPluginMBT.banMutex.release();
                    // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 释放封禁锁 (解禁)。`);
                }
            }
        }
    } catch (error) {
    ...
    if (configChanged && saved) {
        // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 配置已更改且保存成功，准备后台应用...`);
        setImmediate(async () => {
          // logger.info(`${logPrefix} [${actionVerb}][诊断 V1.32] setImmediate 回调开始，准备应用封禁列表...`);
          try {
            // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 调用 GenerateAndApplyBanList...`);
            await MiaoPluginMBT.GenerateAndApplyBanList(MiaoPluginMBT._imgDataCache, logger);
            logger.info(`${logPrefix} [${actionVerb}] 操作后，后台已重新应用生效封禁列表。`);
            if (needsRestore) {
                // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 尝试恢复文件...`);
                const restored = await MiaoPluginMBT.RestoreFileFromSource(targetRelativePath, logger);
                if (!restored) {
                  logger.warn(`${logPrefix} [解禁] 尝试恢复 ${targetFileName} 失败 (可能源文件丢失)。`);
                } else {
                   logger.info(`${logPrefix} [解禁] 文件 ${targetFileName} 已尝试恢复。`);
                }
            }
          } catch (err) {
            logger.error(`${logPrefix} [${actionVerb}] 后台应用生效列表或恢复文件时出错:`, err);
            await this.ReportError(e, `${actionVerb}图片 (后台任务)`, err);
          }
          // logger.info(`${logPrefix} [${actionVerb}][诊断 V1.32] setImmediate 回调结束。`);
        });
    }
    // logger.debug(`${logPrefix} [${actionVerb}][诊断 V1.32] 函数处理结束。`);
  }
   ...
   */
   async FindRoleSplashes(e) {
    ...
    try {
      ...
      const rawRoleImageData = MiaoPluginMBT._imgDataCache.filter(img => img.characterName === standardMainName);
      ...
      const config = MiaoPluginMBT.MBTConfig;
      const roleImageData = MiaoPluginMBT.FilterImagesBySwitches(rawRoleImageData, config);

      if (roleImageData.length === 0) {
          return e.reply(`根据当前的设置，没有找到『${standardMainName}』的可用图片。`, true);
      }
      ...
      for (let batchNum = 1; batchNum <= totalBatches; batchNum++) {
        ...
        for (let i = 0; i < currentBatchData.length; i++) {
          ...
          const isEffectivelyBanned = MiaoPluginMBT._activeBanSet.has(normalizedPath);
  ...
  async VisualizeRoleSplashes(e) {
    ...
    try {
      ...
      try {
        ...
        await this.ReportError(e, `可视化角色 ${standardMainName}`, readErr, '读取角色文件夹失败');
        return true;
      }
        ...
        try {
          ...
          try {
            img = await puppeteer.screenshot(`guguniu-visualize-${standardMainName}-batch${batchNum}`, {
             ...
             logger.error(`${logPrefix} [可视化] Puppeteer 生成第 ${batchNum}/${totalBatches} 批截图失败:`, screenshotError);
          ...
          if (tempImgFilePath && fs.existsSync(tempImgFilePath)) {
           ...
           const possiblePuppeteerTempDir = path.join(MiaoPluginMBT.paths.tempPath, '..', `guguniu-visualize-${standardMainName}-batch${batchNum}`);
      ...
      if (standardMainName) {
        const possibleGenericDir = path.join(MiaoPluginMBT.paths.tempPath, '..', `guguniu-visualize-${standardMainName}`);
        if (fs.existsSync(possibleGenericDir)) {
            try { await safeDelete(possibleGenericDir); } catch (deleteErr) {}
        }
      }
      // logger.debug(`${logPrefix} [可视化] VisualizeRoleSplashes 执行完毕.`); //调试日志专用
    }
    return true;
  }
  ...
  async ExportSingleImage(e) {
    ...
    try {
      ...
      try {
        ...
        await this.ReportError(e, `导出文件 ${targetFileName}`, readError, '读取文件失败');
        return true;
      }
  ...
  async ManualTestProxies(e) {
    ...
    try {
      try {
          ...
          // logger.debug(`${logPrefix} [手动测速] 使用外部模板: ${sourceHtmlPath}`);
      } catch (err) {
          logger.error(`${logPrefix} [手动测速] 找不到外部模板文件: ${sourceHtmlPath}`, err);
      ...
      // logger.debug(`${logPrefix} [手动测速] [诊断 V1.28] 准备展开传递的数据 (renderData):`, renderData);

      await fsPromises.mkdir(MiaoPluginMBT.paths.tempHtmlPath, { recursive: true });
      tempHtmlFilePath = path.join(
        MiaoPluginMBT.paths.tempHtmlPath,
        `manual-speedtest-temp-${Date.now()}-${Math.random().toString(16).slice(2)}.html`
      );
      try {
          await fsPromises.copyFile(sourceHtmlPath, tempHtmlFilePath);
          // logger.debug(`${logPrefix} [手动测速] 已将外部模板复制到临时文件: ${tempHtmlFilePath}`);
      } catch (copyErr) {
          logger.error(`${logPrefix} [手动测速] 复制模板文件失败:`, copyErr);
          await this.ReportError(e, '手动网络测速', copyErr, '无法创建临时模板文件');
          return true;
      }
      ...
      // logger.info(`${logPrefix} [手动测速] [诊断 V1.28] 准备调用 puppeteer.screenshot (tplFile + ...data 方式)...`);
      // logger.debug(`${logPrefix} [手动测速] [诊断 V1.28] tplFile: ${tempHtmlFilePath}`);
      ...
      // logger.info(`${logPrefix} [手动测速] [诊断 V1.28] puppeteer.screenshot 调用完成，返回类型: ${typeof img}`);


      if (img) {
        await e.reply(img);
      } else {
        logger.error(`${this.logPrefix} [手动测速] 生成截图失败 (Puppeteer 返回空)。`);
   ...
   */
    async ShowSettingsPanel(e, extraMsg = '') {
      ...
      try {
          try {
              await fsPromises.access(sourceHtmlPath);
              // logger.debug(`${logPrefix} [设置面板] 找到模板文件: ${sourceHtmlPath}`);
          } catch (err) {
          ...
          const renderData = {
              ...
              tuKuStatus: { text: tuKuEnabled ? '已启用' : '已禁用', class: tuKuEnabled ? 'value-enabled' : 'value-disabled' },
              pflStatus: { level: pflLevel, description: Purify_Level.getDescription(pflLevel), class: `value-level-${pflLevel}` },
              aiStatus: { text: aiEnabled ? '已开启' : '已关闭', class: aiEnabled ? 'value-enabled' : 'value-disabled' },
              easterEggStatus: { text: easterEggEnabled ? '已开启' : '已关闭', class: easterEggEnabled ? 'value-enabled' : 'value-disabled' },
              layoutStatus: { text: layoutEnabled ? '已开启' : '已关闭', class: layoutEnabled ? 'value-enabled' : 'value-disabled' },
              scaleStyleValue: scaleStyleValue,
              backgroundImageUrl: randomBgImageUrl,
          };
          // logger.debug(`${logPrefix} [设置面板] [诊断] 准备展开传递给截图的数据:`, renderData);
  
          await fsPromises.mkdir(MiaoPluginMBT.paths.tempHtmlPath, { recursive: true });
          tempHtmlFilePath = path.join(MiaoPluginMBT.paths.tempHtmlPath, `settings-panel-tpl-${Date.now()}.html`);
          await fsPromises.copyFile(sourceHtmlPath, tempHtmlFilePath);
          // logger.debug(`${logPrefix} [设置面板] [诊断] 已复制模板到临时文件: ${tempHtmlFilePath}`);
  
  
          await fsPromises.mkdir(MiaoPluginMBT.paths.tempImgPath, { recursive: true });
          tempImgFilePath = path.join(MiaoPluginMBT.paths.tempImgPath, `settings-${Date.now()}.png`);
  
          logger.info(`${logPrefix} [设置面板] 开始生成设置面板截图 (tplFile + ...data 方式)...`);
          // logger.debug(`${logPrefix} [设置面板] [诊断] tplFile: ${tempHtmlFilePath}`);
          const screenshotData = { ...renderData }; // 组合最终数据
          // logger.debug(`${logPrefix} [设置面板] [诊断] data:`, screenshotData);
          ...
          const img = await puppeteer.screenshot('guguniu-settings-panel', {
              ...
              imgType: 'png',
              pageGotoParams: { waitUntil: 'networkidle0' },
              ...screenshotData, // 直接展开数据
              screenshotOptions: { fullPage: true  },
              pageBoundingRect: { selector: '.panel', padding: 15},
              width: 480
          });
          // logger.info(`${logPrefix} [设置面板] [诊断] puppeteer.screenshot 调用完成，返回类型: ${typeof img}`);
          ...
          if (img) {
              ...
              logger.error(`${logPrefix} [设置面板] Puppeteer 未能成功生成图片 (返回空)。`);
          ...
          if (error.message?.includes('Cannot read properties of undefined')) {
               logger.error(`${logPrefix} [设置面板] [诊断] 截图报错 'Cannot read properties of undefined'，请再次确认模板文件 (${sourceHtmlPath}) 中的变量名 (如 tuKuStatus.class) 是否与代码中 renderData 的结构完全一致！`);
          }
    ...
    try {
      switch (errorType) {
        ...
        case 'data':
          ...
          throw mockError
        case 'ref':
          mockError = new ReferenceError('模拟引用错误: someUndefinedVariable is not defined')
          console.log(someUndefinedVariable)
          break
        case 'type':
          mockError = new TypeError('模拟类型错误: (intermediate value).iDontExist is not a function')
          ;(123).iDontExist()
          break
        case 'Repo1':
  ...
  async HandleSettingsCommand(e) {
    ...
    const match = e.msg.match(/^#咕咕牛设置(ai|彩蛋|横屏|净化等级)([012]|开启|关闭)$/i);
    if (!match || !match[1] || !match[2]) {
        this.logger.warn(`${this.logPrefix} [统一设置] 正则匹配异常? msg: ${e.msg}`);
        return false;
    }
    ...
    // logger.debug(`${logPrefix} [统一设置][诊断] featureKeyRaw: ${featureKeyRaw}, valueRaw: ${valueRaw}`);

    if (featureKeyRaw === '净化等级') {
      const level = parseInt(valueRaw, 10);
      if (isNaN(level) || ![0, 1, 2].includes(level)) {
        return e.reply(`无效的净化等级值: ${valueRaw}，只能是 0, 1, 或 2。`, true);
      }
      // logger.debug(`${logPrefix} [统一设置][诊断] 准备调用 setPurificationLevelInternal, level: ${level}`);
      ...
      // logger.debug(`${logPrefix} [统一设置][诊断] 准备调用 handleSwitchCommand, configKey: ${configKey}, featureName: ${featureName}, enable: ${enable}`);
  ...
  async setPurificationLevelInternal(e, level) {
    ...
    // logger.debug(`${logPrefix} [净化设置内部][诊断] 开始处理，请求等级: ${level}`);
    await MiaoPluginMBT.configMutex.acquire();
    // logger.debug(`${logPrefix} [净化设置内部][诊断] 已获取配置锁。`);
    try {
        await MiaoPluginMBT.LoadTuKuConfig(true, logger);
        const currentLevel = MiaoPluginMBT.MBTConfig.PFL ?? Default_Config.defaultPfl;
        // logger.debug(`${logPrefix} [净化设置内部][诊断] 当前等级: ${currentLevel}`);

        if (level === currentLevel) {
           logger.info(`${logPrefix} [净化设置] 等级未变，尝试显示面板。`);
           try {
               await this.ShowSettingsPanel(e, `净化等级已经是 ${level} (${Purify_Level.getDescription(level)}) 啦。`);
           } catch (panelError) {
               logger.error(`${logPrefix} [净化设置] 显示当前状态面板失败，发送文本回退:`, panelError);
               await e.reply(`${logPrefix} 净化等级已经是 ${level} (${Purify_Level.getDescription(level)}) 啦。`, true);
           }
           MiaoPluginMBT.configMutex.release();
           // logger.debug(`${logPrefix} [净化设置内部][诊断] 释放配置锁 (状态未变路径)。`);
           return;
        }
        ...
        // logger.debug(`${logPrefix} [净化设置内部][诊断] 尝试保存配置...`);
        const saveSuccess = await MiaoPluginMBT.SaveTuKuConfig(MiaoPluginMBT.MBTConfig, logger);
        if (!saveSuccess) {
            ...
            MiaoPluginMBT.configMutex.release();
            // logger.debug(`${logPrefix} [净化设置内部][诊断] 释放配置锁 (保存失败路径)。`);
            return;
        }
        // logger.debug(`${logPrefix} [净化设置内部][诊断] 配置保存成功。`);
    } catch (configError) {
        logger.error(`${logPrefix} [净化设置] 处理配置时出错:`, configError);
        await this.ReportError(e, '设置净化等级', configError, '处理配置时出错');
        MiaoPluginMBT.configMutex.release();
        // logger.debug(`${logPrefix} [净化设置内部][诊断] 释放配置锁 (配置处理错误路径)。`);
        return;
    } finally {
        MiaoPluginMBT.configMutex.release();
        // logger.debug(`${logPrefix} [净化设置内部][诊断] 释放配置锁 (finally 块)。`);
    }

    if (configChanged) {
        // logger.debug(`${logPrefix} [净化设置内部][诊断] 配置已更改，准备后台应用...`);
        setImmediate(async () => {
          // logger.debug(`${logPrefix} [净化设置内部][诊断] setImmediate 回调开始。`);
          try {
            logger.info(`${logPrefix} [净化设置] 后台开始应用新的净化等级 ${level}...`);
            // logger.debug(`${logPrefix} [净化设置内部][诊断] 调用 GenerateAndApplyBanList...`);
            await MiaoPluginMBT.GenerateAndApplyBanList(MiaoPluginMBT._imgDataCache, logger);
            logger.info(`${logPrefix} [净化设置] 新的生效封禁列表已应用。`);
            if (MiaoPluginMBT.MBTConfig.TuKuOP) {
                logger.info(`${logPrefix} [净化设置] 图库已启用，开始重新同步角色文件夹...`);
                // logger.debug(`${logPrefix} [净化设置内部][诊断] 调用 SyncCharacterFolders...`);
                await MiaoPluginMBT.SyncCharacterFolders(logger);
            ...
            await this.ReportError(e, '应用净化等级 (后台)', applyError);
              ...
              // logger.debug(`${logPrefix} [净化设置内部][诊断] setImmediate 回调结束，尝试显示面板。`);
              let panelSent = false;
              try {
                  let extraPanelMsg = '';
                  if (asyncError) extraPanelMsg += `\n(后台应用时遇到问题)`;
                  ...
                  await this.ShowSettingsPanel(e, extraPanelMsg.trim());
                  panelSent = true;
                  // logger.debug(`${logPrefix} [净化设置内部][诊断] 最终面板显示成功。`);
              } catch (panelError) {
                  logger.error(`${logPrefix} [净化设置] 显示设置面板失败，将发送文本回退:`, panelError);
                  panelSent = false;
                  let finalUserMessage = `${logPrefix} 净化等级已设为 ${level} (${Purify_Level.getDescription(level)})。`;
    ...
    // logger.debug(`${logPrefix} [净化设置内部][诊断] 处理结束。`);
  }
  ...
  async handleSwitchCommand(e, configKey, featureName, enable) {
    const logger = this.logger;
    const logPrefix = this.logPrefix;
    // logger.debug(`${logPrefix} [${featureName}设置][诊断] 进入 handleSwitchCommand 函数, enable: ${enable}`);

    let configChanged = false;
    let saveWarning = '';
    let asyncError = null;

    // logger.debug(`${logPrefix} [${featureName}设置][诊断] 尝试获取配置锁...`);
    await MiaoPluginMBT.configMutex.acquire();
    // logger.debug(`${logPrefix} [${featureName}设置][诊断] 已获取配置锁。`);
    try {
        // logger.debug(`${logPrefix} [${featureName}设置][诊断] 加载最新配置...`);
        await MiaoPluginMBT.LoadTuKuConfig(true, logger);
        const currentStatus = MiaoPluginMBT.MBTConfig[configKey];
        // logger.debug(`${logPrefix} [${featureName}设置][诊断] 当前状态: ${currentStatus}`);

        if (currentStatus === enable) {
            logger.info(`${logPrefix} [${featureName}设置] 当前状态已是 ${enable ? '开启' : '关闭'}，无需更改。`);
            try {
                await this.ShowSettingsPanel(e, `${featureName}已经是「${enable ? '开启' : '关闭'}」状态了。`);
            } catch (panelError) {
                logger.error(`${logPrefix} [${featureName}设置] 显示当前状态面板失败，发送文本回退:`, panelError);
                await e.reply(`${logPrefix} ${featureName}已经是「${enable ? '开启' : '关闭'}」状态了，无需更改。`, true);
            }
            MiaoPluginMBT.configMutex.release();
            // logger.debug(`${logPrefix} [${featureName}设置][诊断] 释放配置锁 (状态未变路径)。`);
            return;
        }

        MiaoPluginMBT.MBTConfig[configKey] = enable;
        configChanged = true;
        logger.info(`${logPrefix} [${featureName}设置] 内存状态变更为 -> ${enable}`);

        // logger.debug(`${logPrefix} [${featureName}设置][诊断] 尝试保存配置...`);
        const saveSuccess = await MiaoPluginMBT.SaveTuKuConfig(MiaoPluginMBT.MBTConfig, logger);
        if (!saveSuccess) {
            saveWarning = `⚠️ 但是配置保存失败了！设置可能不会持久生效。`;
            MiaoPluginMBT.MBTConfig[configKey] = !enable;
            ...
            MiaoPluginMBT.configMutex.release();
            // logger.debug(`${logPrefix} [${featureName}设置][诊断] 释放配置锁 (保存失败路径)。`);
            return;
        }
        // logger.debug(`${logPrefix} [${featureName}设置][诊断] 配置保存成功。`);
    } catch (configError) {
        logger.error(`${logPrefix} [${featureName}设置] 处理配置时出错:`, configError);
        await this.ReportError(e, `设置${featureName}`, configError, '处理配置时出错');
        MiaoPluginMBT.configMutex.release();
        // logger.debug(`${logPrefix} [${featureName}设置][诊断] 释放配置锁 (配置处理错误路径)。`);
        return;
    } finally {
        MiaoPluginMBT.configMutex.release();
        // logger.debug(`${logPrefix} [${featureName}设置][诊断] 释放配置锁 (finally 块)。`);
    }
    ...
    if (configChanged) {
        ...
        try {
            let extraPanelMsg = '';
            if (saveWarning) extraPanelMsg += `\n${saveWarning}`;
            // logger.debug(`${logPrefix} [${featureName}设置][诊断] 尝试显示最终面板...`);
            await this.ShowSettingsPanel(e, extraPanelMsg.trim());
            panelSent = true;
            // logger.debug(`${logPrefix} [${featureName}设置][诊断] 最终面板显示成功。`);
        } catch (panelError) {
            logger.error(`${logPrefix} [${featureName}设置] 显示设置面板失败，将发送文本回退:`, panelError);
            panelSent = false;
            let finalUserMessage = `${logPrefix} ${featureName}已设为「${enable ? '开启' : '关闭'}」。`;
            if (saveWarning) finalUserMessage += `\n${saveWarning}`;
            await e.reply(finalUserMessage, true);
        }
    }
    // logger.debug(`${logPrefix} [${featureName}设置][诊断] 函数处理结束。`);
  }
    ...
    MiaoPluginMBT.initializationPromise = (async () => {
      ...
      try {
        ...
        try {
          ...
          MiaoPluginMBT._backgroundImages = files.filter(file => /\.webp$/i.test(file));
          if (MiaoPluginMBT._backgroundImages.length > 0) {
              logger.info(`${logPrefix} [初始化] 加载了 ${MiaoPluginMBT._backgroundImages.length} 张背景图`);
          }
        } catch (err) {
            if (err.code !== ERROR_CODES.NotFound) {
                ...
                try {
                    ...
                    if (err.code !== ERROR_CODES.NotFound) {
                        logger.error(`${logPrefix} [延迟清理] 删除旧插件文件 (${oldPluginPath}) 时出错:`, err);
                         ...
                         // logger.debug(`${logPrefix} [延迟清理] 未检测到旧插件文件，无需清理。`); //调试日志专用
                    }
                }
            }, delaySeconds * 1000);
        }
        ...
        // logger.debug(`${logPrefix} [初始化] 同步锁已释放。`); //调试日志专用
      }
    })();
  ...
  static async LoadTuKuConfig(
    ...
    try {
        if (!forceReload && MiaoPluginMBT.MBTConfig && Object.keys(MiaoPluginMBT.MBTConfig).length > 0) {
          // logger.debug(`${logPrefix} [加载配置] 使用内存缓存。`); //调试日志专用
          return MiaoPluginMBT.MBTConfig;
        }

        // logger.debug(`${logPrefix} [加载配置] 尝试从文件加载: ${configPath}`); //调试日志专用
        let configData = {};
        try {
          await fsPromises.access(configPath);
          // logger.debug(`${logPrefix} [加载配置] 文件存在，准备读取...`); //调试日志专用
          const content = await fsPromises.readFile(configPath, 'utf8');
          // logger.debug(`${logPrefix} [加载配置] 文件读取完成，准备解析 YAML...`); //调试日志专用
          configData = yaml.parse(content) || {};
          // logger.debug(`${logPrefix} [加载配置] YAML 解析完成。`); //调试日志专用
        } catch (error) {
        ...
        logger.info(
          `${logPrefix} [加载配置] 完成: 图库=${loadedConfig.TuKuOP ? '开' : '关'}, PFL=${
            loadedConfig.PFL
          }, Ai=${loadedConfig.Ai}, 彩蛋=${loadedConfig.EasterEgg}, 横屏=${loadedConfig.layout}`
        );
        return MiaoPluginMBT.MBTConfig;
    } finally {
        // 无锁操作
  ...
  static async SaveTuKuConfig(configData, logger = global.logger || console) {
    ...
    try {
        ...
        // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] 准备保存的数据:`, dataToSave); //调试日志专用

        const dirPath = path.dirname(configFilePath);
        // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] 准备创建目录: ${dirPath}`); //调试日志专用
        try {
            await fsPromises.mkdir(dirPath, { recursive: true });
            // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] 目录创建完成/已存在。`); //调试日志专用
        } catch (mkdirError) {
            logger.error(`${logPrefix} [保存配置] 创建目录 ${dirPath} 失败:`, mkdirError);
            return false;
        }

        // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] 准备序列化 YAML...`); //调试日志专用
        const yamlString = yaml.stringify(dataToSave);
        // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] YAML 序列化完成。准备写入文件: ${configFilePath}`); //调试日志专用
        // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] 即将执行 await fsPromises.writeFile...`); //调试日志专用

        await fsPromises.writeFile(configFilePath, yamlString, 'utf8');
        // logger.info(`${logPrefix} [保存配置] [诊断 V1.21] fsPromises.writeFile 调用完成。`); //调试日志专用

        logger.info(`${logPrefix} [保存配置] 成功保存配置到 ${configFilePath}`);

        MiaoPluginMBT.MBTConfig = { ...MiaoPluginMBT.MBTConfig, ...configData };
        // logger.debug(`${logPrefix} [保存配置] [诊断 V1.21] 内存配置已更新。`); //调试日志专用
        return true;
    } catch (error) {
      logger.error(
  ...
  static async ScanLocalImagesToBuildCache(logger = global.logger || console) {
    ...
    for (const storageBoxName in repoPaths) {
        ...
        const repoNum = storageBoxName === 'Miao-Plugin-MBT' ? 1 : (storageBoxName === 'Miao-Plugin-MBT-2' ? 2 : 0);
  ...
  static GenerateBanList(imageData, logger = global.logger || console) {
    ...
    if (pflLevel > Purify_Level.NONE && Array.isArray(imageData) && imageData.length > 0) {
      ...
      logger.warn(`${Default_Config.logPrefix} [生成封禁] PFL=${pflLevel} 但元数据无效或为空，无法执行 PFL 净化。`);
    }
    ...
    logger.info(
      `${Default_Config.logPrefix} [生成封禁] 等级PFL=${pflLevel} (${Purify_Level.getDescription(pflLevel)}), 开关(Ai:${!filterAi},彩蛋:${!filterEasterEgg},横屏:${!filterLayout})`
    );
    logger.info(
  ...
  static getRandomBgImageUrl() {
    if (!MiaoPluginMBT._backgroundImages || MiaoPluginMBT._backgroundImages.length === 0) {
        return '';
    }
    const randomIndex = Math.floor(Math.random() * MiaoPluginMBT._backgroundImages.length);
  ...
  static async LoadUserBans(forceReload = false, logger = global.logger || console) {
    if (MiaoPluginMBT._userBanSet instanceof Set && MiaoPluginMBT._userBanSet.size > 0 && !forceReload) {
      return true
    }
    let data = []
    let success = false
  ...
  static async SaveUserBans(logger = global.logger || console) {
    ...
    // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 尝试获取封禁锁...`); //调试日志专用
    await MiaoPluginMBT.banMutex.acquire();
    // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 已获取封禁锁。`); //调试日志专用
    try {
        const sortedBans = Array.from(MiaoPluginMBT._userBanSet).sort();
        // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 准备保存 ${sortedBans.length} 条记录。`); //调试日志专用
        const jsonString = JSON.stringify(sortedBans, null, 2);

        const dirPath = path.dirname(banListPath);
        // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 准备创建目录: ${dirPath}`); //调试日志专用
        try {
            await fsPromises.mkdir(dirPath, { recursive: true });
            // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 目录创建完成/已存在。`); //调试日志专用
        } catch (mkdirError) {
            logger.error(`${logPrefix} [保存用户封禁] 创建目录 ${dirPath} 失败:`, mkdirError);
            return false;
        }

        // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 准备写入文件: ${banListPath}`); //调试日志专用
        // logger.info(`${logPrefix} [保存用户封禁][诊断 V1.30] 即将执行 await fsPromises.writeFile...`); //调试日志专用
        await fsPromises.writeFile(banListPath, jsonString, 'utf8');
        // logger.info(`${logPrefix} [保存用户封禁][诊断 V1.30] fsPromises.writeFile 调用完成。`); //调试日志专用

        logger.info(`${logPrefix} [保存用户封禁] 成功保存配置到 ${banListPath}`);
        return true;
    } catch (error) {
      logger.error(
        `${logPrefix} [保存用户封禁] 写入配置文件 ${banListPath} 失败:`,
        error
      );
      return false;
    } finally {
        MiaoPluginMBT.banMutex.release();
        // logger.debug(`${logPrefix} [保存用户封禁][诊断 V1.30] 释放封禁锁。`); //调试日志专用
    }
  }
  /**
   ...
   */
   static async FindImageAbsolutePath(relativePath) {
    ...
    const logger = global.logger || console;
    const reposToSearch = [
        { name: 'Miao-Plugin-MBT', path: MiaoPluginMBT.paths.LocalTuKuPath, num: 1 },
        { name: 'Miao-Plugin-MBT-2', path: MiaoPluginMBT.paths.LocalTuKuPath2, num: 2 }
    ...
    for (const repo of reposToSearch) {
        ...
        if (repoExists) {
          ...
          try {
            ...
            if (err.code !== ERROR_CODES.NotFound) {
              // logger.warn(`${Default_Config.logPrefix} [查找路径] 访问仓库 ${repo.name} (${absPath}) 出错:`, err.code); //调试日志专用
            }
          }
        }
    }
  ...
  static async DownloadRepoWithFallback(repoNum, repoUrl, branch, localPath, eForProgress, loggerInstance) {
    ...
    // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 进入函数 @ ${functionStartTime}`); //调试日志专用
    ...
    try {
      // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 开始测速 @ ${testStartTime}`); //调试日志专用
      allTestResults = await MiaoPluginMBT.TestProxies(baseRawUrl, loggerInstance);
      // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 测速结束 @ ${Date.now()}`); //调试日志专用
    } catch (testError) {
      loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 代理测速失败:`, testError);
    ...
    if (eForProgress && repoNum === 1 && allTestResults.length > 0) {
        ...
        try {
            ...
            if (bestSourceForReport) {
                ...
                if (bestSourceForReport.testUrlPrefix !== null) {
                    speedInfo = (Number.isFinite(bestSourceForReport.speed) && bestSourceForReport.speed >= 0)
            ...
            htmlContent = template.render(SPEEDTEST_HTML_TEMPLATE_LOCAL, renderData);
            // loggerInstance.debug(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [诊断] 预渲染 HTML 内容类型: ${typeof htmlContent}, 是否为空: ${!htmlContent}`); //调试日志专用
            ...
            tempHtmlFilePath = path.join(
                MiaoPluginMBT.paths.tempHtmlPath,
                `dl-speedtest-rendered-${Date.now()}-${Math.random().toString(16).slice(2)}.html`
            );
            await fsPromises.writeFile(tempHtmlFilePath, htmlContent, 'utf8');
            // loggerInstance.debug(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 已将渲染好的 HTML 写入临时文件: ${tempHtmlFilePath}`); //调试日志专用
            canGenerateReport = true;

        } catch (prepOrRenderError) {
             loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 准备或渲染测速报告失败:`, prepOrRenderError);
             await eForProgress.reply(`${logPrefix} 准备或渲染测速报告出错 (${prepOrRenderError.message?.substring(0,50)}...)，继续下载...`).catch(() => {});
        ...
        if (canGenerateReport) {
            try {
                ...
                // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [诊断] 准备调用 puppeteer.screenshot (预渲染 tplFile 方式)...`); //调试日志专用
                // loggerInstance.debug(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [诊断] tplFile: ${tempHtmlFilePath}`); //调试日志专用
                ...
                // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [诊断] puppeteer.screenshot 调用完成，返回类型: ${typeof img}`); //调试日志专用

                if (img) {
                    await eForProgress.reply(img); await common.sleep(500);
                } else {
                    loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 生成测速截图成功，但 Puppeteer 返回空 (可能为空白图片)。`);
                    await eForProgress.reply(`${logPrefix} 生成测速报告图片时内容可能为空，继续下载...`).catch(() => {});
                }
            } catch (screenshotError) {
                loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] Puppeteer 生成测速截图失败:`, screenshotError);
                if (screenshotError.message?.includes('加载html错误：undefined')) {
                    loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [诊断] 即使使用预渲染 tplFile 方式仍然报错 '加载html错误：undefined'，问题可能在截图组件或环境。`);
                }
                await eForProgress.reply(`${logPrefix} 生成测速报告截图出错 (${screenshotError.message?.substring(0,50)}...)，继续下载...`).catch(() => {});
            } finally {
    ...
    if (githubResult && githubResult.speed !== Infinity && githubResult.speed <= 300) {
        ...
        loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 检测到 GitHub 直连延迟 (${githubResult.speed}ms) <= 300ms，优先尝试直连...`);
        ...
        // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 尝试获取 Git 锁 (GitHub 直连) @ ${Date.now()}`); //调试日志专用
        await MiaoPluginMBT.gitMutex.acquire();
        // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 已获取 Git 锁 (GitHub 直连) @ ${Date.now()}`); //调试日志专用
        try {
            // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 开始 ExecuteCommand (GitHub 直连) @ ${Date.now()}`); //调试日志专用
            ...
            // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] ExecuteCommand 成功 (GitHub 直连) @ ${Date.now()}`); //调试日志专用
            loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 使用 ${nodeName} 下载成功！`);
            // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 函数成功返回 @ ${Date.now()}`); //调试日志专用
            return { success: true, nodeName: nodeName };
        } catch (error) {
             // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] ExecuteCommand 失败 (GitHub 直连) @ ${Date.now()}`); //调试日志专用
             loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 优先尝试 ${nodeName} 下载失败。将继续尝试其他源...`);
             loggerInstance.error(error);
             lastError = error;
             loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 尝试清理直连失败产生的目录: ${localPath}`);
             await safeDelete(localPath);
             await common.sleep(1000);
        } finally {
            MiaoPluginMBT.gitMutex.release();
            // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 释放 Git 锁 (GitHub 直连) @ ${Date.now()}`); //调试日志专用
        }
    } else if (githubResult) {
         loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] GitHub 直连延迟 (${githubResult.speed === Infinity ? '超时/失败' : githubResult.speed + 'ms'}) > 300ms，按优先级选择节点。`);
    } else {
         loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 未找到 GitHub 测速结果，按优先级选择节点。`);
    }
    ...
    if (sourcesToTry.length === 0) {
      loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 没有任何可用的下载源！(包括 GitHub 直连尝试后)`);
      if (repoNum === 1 && eForProgress) {
          await MiaoPluginMBT.ReportError(eForProgress, `下载${repoTypeName}`, lastError || new Error('无可用下载源'), `测速结果: ${JSON.stringify(allTestResults)}`, loggerInstance);
      }
      // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 函数失败返回 @ ${Date.now()}`); //调试日志专用
      return { success: false, nodeName: '无可用源', error: lastError || new Error('无可用下载源') };
    }

    loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 开始尝试列表: ${sourcesToTry.map(s => s.name).join(', ')}`);

    for (const source of sourcesToTry) {
      if (source.name === 'GitHub' && githubDirectAttempted) {
           // loggerInstance.debug(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 已优先尝试过 GitHub 直连，跳过本次列表中的 GitHub 源。`); //调试日志专用
           continue;
      }

      const nodeName = source.name === 'GitHub' ? 'GitHub(直连)' : `${source.name}(代理)`;
      loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 尝试使用源: ${nodeName}`);
       ...
       if (source.name === 'GitHub') { cloneUrl = repoUrl; }
         ...
         try { const proxyUrl = new URL(source.cloneUrlPrefix); if (['http:', 'https:'].includes(proxyUrl.protocol)) proxyForEnv = proxyUrl.origin; }
         catch (urlError) { loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 无法解析代理 ${source.name} 的 cloneUrlPrefix: ${urlError.message}`); }
       } else { loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 源 ${source.name} 没有 cloneUrlPrefix，跳过。`); continue; }
       ...
       if (proxyForEnv) { gitOptions.env = { ...process.env, HTTP_PROXY: proxyForEnv, HTTPS_PROXY: proxyForEnv }; loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 为 Git 命令设置代理: ${proxyForEnv}`); }


      // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 尝试获取 Git 锁 (${nodeName}) @ ${Date.now()}`); //调试日志专用
      await MiaoPluginMBT.gitMutex.acquire();
      // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 已获取 Git 锁 (${nodeName}) @ ${Date.now()}`); //调试日志专用
      try {
        // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 开始 ExecuteCommand (${nodeName}) @ ${Date.now()}`); //调试日志专用
        ...
        // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] ExecuteCommand 成功 (${nodeName}) @ ${Date.now()}`); //调试日志专用
        loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 使用源 ${nodeName} 下载成功！`);
        // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 函数成功返回 @ ${Date.now()}`); //调试日志专用
        return { success: true, nodeName: nodeName };
      } catch (error) {
         // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] ExecuteCommand 失败 (${nodeName}) @ ${Date.now()}`); //调试日志专用
         loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 使用源 ${nodeName} 下载失败。`);
         loggerInstance.error(error);
         lastError = error;
         loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 尝试清理失败下载产生的目录: ${localPath}`);
         await safeDelete(localPath);
         await common.sleep(1000);
         loggerInstance.warn(`${logPrefix} 使用源 ${nodeName} 下载 ${repoTypeName} (${repoNum}号) 失败，尝试下一个源...`);
      } finally {
          MiaoPluginMBT.gitMutex.release();
          // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] [计时] 释放 Git 锁 (${nodeName}) @ ${Date.now()}`); //调试日志专用
      }
    }

     loggerInstance.error(
       `${logPrefix} [下载流程 ${repoTypeName} (${repoNum}号)] 尝试了所有可用源 (${sourcesToTry.map(s => s.name).join(', ')})，均下载失败！`
     );
     if (repoNum === 1 && eForProgress) { // 只有核心仓库失败时才向用户报告
         await MiaoPluginMBT.ReportError(eForProgress, `下载${repoTypeName}`, lastError || new Error('所有源下载失败'), `尝试源: ${sourcesToTry.map(s => s.name).join(', ')}`, loggerInstance);
     } else {
  ...
  static async UpdateSingleRepo(e, RepoNum, localPath, RepoName, RepoUrl, branch, isScheduled, logger) {
    ...
    try {
      let oldCommit = '';
      try { oldCommit = (await ExecuteCommand('git', ['rev-parse', 'HEAD'], { cwd: localPath }, 5000)).stdout.trim(); }
      catch {}
      ...
      try {
        const pullResult = await ExecuteCommand(
          ...
          undefined,
          stderrChunk => {
              const matchLog = stderrChunk.match(/(Receiving objects|Resolving deltas):\s*(\d+)%/);
              if (matchLog) logger.debug(`${logPrefix} [更新进度 ${RepoName}] ${matchLog[1]}: ${matchLog[2]}%`);
          }
        );
        ...
        logger.info(`${logPrefix} [更新仓库] ${RepoName} 'git pull --ff-only' 成功。`);
         ...
         if (err.code !== 0 && (err.stderr?.includes('commit') || err.stderr?.includes('unrelated') || err.stderr?.includes('lock') || err.stderr?.includes('fast-forward') || err.message?.includes('failed'))) {
           needsReset = true;
      ...
      if (success && !needsReset) {
         let newCommit = '';
         try { newCommit = (await ExecuteCommand('git', ['rev-parse', 'HEAD'], { cwd: localPath }, 5000)).stdout.trim(); }
         catch {}
         hasChanges = oldCommit && newCommit && oldCommit !== newCommit;
         if (hasChanges) { logger.info(`${Default_Config.logPrefix} [更新仓库] ${RepoName} 检测到新的提交。`); latestLog = await MiaoPluginMBT.GetTuKuLog(20, localPath, logger); }
         else if (pullOutput.includes('Already up to date')) { logger.info(`${Default_Config.logPrefix} [更新仓库] ${RepoName} 已是最新。`); latestLog = await MiaoPluginMBT.GetTuKuLog(1, localPath, logger); }
         else { logger.warn(`${Default_Config.logPrefix} [更新仓库] ${RepoName} pull 成功但未检测到明确更新，获取最新日志...`); latestLog = await MiaoPluginMBT.GetTuKuLog(1, localPath, logger); }
      }
    } catch (error) {
       success = false;
       hasChanges = false;
       ...
       if (RepoNum === 1 && e && !isScheduled) {
         ...
         await MiaoPluginMBT.ReportError(e, `更新${RepoName}`, errorToReport, `Git输出(部分):\n${pullOutput.substring(0, 500)}`, logger);
       } else { logger.error(error || pullError); }
    } finally {
        MiaoPluginMBT.gitMutex.release();
    }
  ...
  static async SyncCharacterFolders(logger = global.logger || console) {
    ...
    if (!imageDataToSync || imageDataToSync.length === 0) {
      logger.warn(`${Default_Config.logPrefix} [同步角色] 元数据为空，无法同步。`);
      return;
    }
    if (!(MiaoPluginMBT._activeBanSet instanceof Set)) {
         logger.warn(`${Default_Config.logPrefix} [同步角色] 生效封禁列表未初始化或类型错误。`);
    }

    logger.info(`${Default_Config.logPrefix} [同步角色] 开始复制 (${imageDataToSync.length} 条元数据)...`);
    let copied = 0,
      banned = 0,
      missingSource = 0,
      noTarget = 0,
    ...
    for (const imgData of imageDataToSync) {
      ...
      if (!relativePath || !storageBox) {
          logger.warn(`${Default_Config.logPrefix} [同步角色] 跳过无效元数据项: path=${relativePath}, storagebox=${storageBox}`);
          noTarget++;
          continue;
      }
      ...
      if (storageBox === 'Miao-Plugin-MBT') {
          ...
          logger.warn(`${Default_Config.logPrefix} [同步角色] 未知的 storagebox: ${storageBox} for path: ${relativePath}`);
          noTarget++;
          continue;
      }
      if (!sourceBasePath) {
          logger.warn(`${Default_Config.logPrefix} [同步角色] 仓库路径未定义: ${storageBox}`);
          missingSource++;
          continue;
      }
          ...
          (async () => {
            try {
              ...
              if (sourceAccessErr.code === ERROR_CODES.NotFound) {
                ...
                errorCount++;
                logger.warn(`${Default_Config.logPrefix} [同步角色] 访问源文件失败: ${sourcePath}`, sourceAccessErr.code);
              }
            }
          })()
        );
...
const GUGUNIU_RULES = [
  ...
  { reg: /^#咕咕牛触发错误(?:\s*(git|fs|config|data|ref|type|Repo1|Repo2|notify|other))?$/i, fnc: 'TriggerError', permission: 'master',},
  ...
  { reg: /^#咕咕牛设置(ai|彩蛋|横屏|净化等级)([012]|开启|关闭)$/i, fnc: 'HandleSettingsCommand', permission: 'master' },
]