...
const Hestia_State_Symbol = Symbol.for("Yz.CowCoo.MBT.Hestia.State.v2");
const Charon = "『咕咕牛』";
...
class Hestia {
  static #g =
    ...
    (() => {
      ...
      global[Hestia_State_Symbol] = s;
      return s;
    })();

  static get activeGen() {
    return Math.max(this.#g.moduleGen, Moirai.currentGen);
  }

  static advance(targetGen = Moirai.currentGen) {
    this.#g.moduleGen = Math.max(this.#g.moduleGen, targetGen);
    for (const [id, res] of this.#g.activeRes) {
      if (res.activeGen < this.#g.moduleGen - 1) {
    ...
    return this.#g.moduleGen;
  }

  static register(type, id, object, cleanup) {
    const ref = new WeakRef(object);
    const entry = { type, id, ref, cleanup, activeGen: this.activeGen, createdAt: Date.now() };
    this.#g.activeRes.set(id, entry);
    this.#g.registry.register(object, { type, id, cleanup });
    return id;
  }

  static registerStrong(type, id, object, cleanup) {
    const entry = { type, id, object, cleanup, activeGen: this.activeGen, createdAt: Date.now() };
    this.#g.strongRes.set(id, entry);
    return id;
  }

  static unregister(id) {
    this.#g.activeRes.delete(id);
    this.#g.strongRes.delete(id);
  }

  static forceCleanup(id) {
    const res = this.#g.activeRes.get(id) ?? this.#g.strongRes.get(id);
    if (!res) return;
    try {
  ...
  static async reapAsync(targetGen) {
    const toClean = [];
    for (const [id, res] of this.#g.activeRes) {
      if (res.activeGen <= targetGen) toClean.push(id);
    }
    ...
    for (const id of toClean) {
      ...
      if (!res) continue;
      try {
        if (typeof res.cleanup === "function") {
          await Promise.resolve(res.cleanup());
        }
      } catch (e) {}
      this.#g.activeRes.delete(id);
      this.#g.strongRes.delete(id);
  ...
  static snapshot() {
    ...
    for (const [id, res] of this.#g.activeRes) {
      ...
      snap[id] = { type: res.type, activeGen: res.activeGen, alive: !!obj, age: Date.now() - res.createdAt };
    }
    for (const [id, res] of this.#g.strongRes) {
  ...
  static get stats() {
    let alive = 0,
      dead = 0;
    for (const [, res] of this.#g.activeRes) {
      if (res.ref?.deref?.()) alive++;
      else dead++;
    }
    for (const [, res] of this.#g.strongRes) {
      if (res.object) alive++;
      else dead++;
    }
...
class MBTAdapterEnv {
  static isOneBotFamily(e) {
    return ProtocolMod ? ProtocolMod.isOneBotFamily(e) : false;
  }

  static adaptImgType(requested = "png") {
    return ProtocolMod ? ProtocolMod.adaptImgType(requested) : requested;
  }
}
...
class Pheme {
  ...
  static async quote(e, msg) {
    return this.send(e, msg, true);
  }
  ...
  static async noPerm(e) {
    return this.quote(e, "这个操作只有我的主人才能用哦~");
  }
  static async noRepo(e) {
    return this.quote(e, "咕咕牛的图库你还没下载呢！");
  }
  static async initFail(e) {
    return this.quote(e, "咕咕牛管理器初始化失败，请检查后台日志。");
  }
  static async notReady(e) {
    return this.quote(e, "咕咕牛插件核心服务未就绪，大部分功能无法使用。");
  }
  ...
  static async wait(e, msg) {
    return this.quote(e, msg);
  }
}
...
class Metis {
  ...
  async run(taskFn, options = {}) {
    ...
    if (ttl > 0) {
      timeoutTimer = setTimeout(() => {
        ...
        this.logger.debug(`${rid} 任务超时 (设:${ttl}ms|实:${elapsed.toFixed(1)}ms)`);
  ...
  emergencyReset(reason) {
    ...
    if (isHMR) {
      this.logger.debug(`Metis:${this.name} | ${reason}`);
    } else {
...
class Nyx {
  ...
  static _bigramSet(s) {
    return new Set(Array.from({ length: s.length - 1 }, (_, i) => (s.charCodeAt(i) << 16) | s.charCodeAt(i + 1)));
  }
      ...
      socket.on("data", (data) => {
        if (step === 0) {
          if (data[0] === 0x05 && data[1] === 0x00) {
            ...
            const connectReq = Buffer.from([0x05, 0x01, 0x00, 0x01, ...ipParts, (probePort >> 8) & 0xff, probePort & 0xff]);
      ...
      socket.on("data", (data) => {
        if (step === 0) {
          if (data[0] === 0x05 && data[1] === 0x00) {
            ...
            const connectReq = Buffer.concat([Buffer.from([0x05, 0x01, 0x00, 0x03, hostBuf.length]), hostBuf, Buffer.from([(443 >> 8) & 0xff, 443 & 0xff])]);
            socket.write(connectReq);
          } else {
...
class Hermes {
  ...
  static async getHostProfile(Hades = console) {
    if (DockerMod?.getHostProfile) return DockerMod.getHostProfile(Hades);
    const platform = os.platform();
    const release = os.release();
    return { platform, release, isDocker: false, container: { dockerenv: false, containerenv: false, cgroup: null, kubernetes: false, hint: null } };
  }
  ...
  static UdpPulse(host, port = 53, timeout = 2000) {
    return new Promise((resolve) => {
      ...
      const packet = Buffer.from([
        0x12, 0x34, 0x01, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x06, 0x67, 0x6f, 0x6f, 0x67, 0x6c, 0x65, 0x03, 0x63, 0x6f, 0x6d, 0x00, 0x00, 0x01, 0x00, 0x01
      ]);
  ...
  static async request(url, options = {}, redirects = 0) {
    ...
    if (!useProxy) {
      ...
      agent = isHttp ? new http.Agent(baseOpts) : new https.Agent({ ciphers: shuffledCiphers, minVersion: "TLSv1.2", ...baseOpts });
  ...
  static #proxyRequest(url, reqOptions, proxy, options, redirects) {
    ...
    return isHttp ? this.#httpViaProxy(url, reqOptions, proxy, options, redirects) : this.#httpsViaProxy(url, reqOptions, proxy, options, redirects);
  }
  ...
  static async getEnvSnapshot(Hades = console) {
    try {
      ...
      const { v4, v6 } = await Morpheus.withPage(async (page) => {
        ...
        const [v4Result, v6Result] = await Promise.all([detect(HermesMatrix.Sources.IPv4, 4), detect(HermesMatrix.Sources.IPv6, 6)]);
      ...
      const formatPuppResult = (result, family) => {
        ...
        const { data } = result;
        const country = result.country ?? data?.country_code ?? data?.countryCode ?? data?.country ?? data?.country_name ?? "N/A";
  ...
  static async getNativeIPStack(Hades = console) {
    ...
    const [v4Result, v6Result] = await Promise.all([race(HermesMatrix.Sources.IPv4, 4), race(HermesMatrix.Sources.IPv6, 6)]);

    if (v4Result?.data?.geoData && typeof v4Result.data.geoData === "string") {
      try {
        v4Result.data.geoData = JSON.parse(v4Result.data.geoData);
      } catch {}
    }
  ...
  static getLanIps() {
    return Object.values(os.networkInterfaces()).flatMap((iface) => iface.filter((f) => !f.internal).map((f) => f.address));
  }
...
class HermesMatrix {
  ...
  static async race(sources, family, requestFn, Hades = null, timeout = 3000) {
    const promises = sources.map((src) =>
      requestFn(src.url, { timeout, family }).then((res) => {
...
class Proteus {
  ...
  static async sense(envData, mirrorSpeed = Infinity, Hades = console) {
    const v6StatePromise =
      envData && (envData.v4Ip || envData.v6Ip)
  ...
  static async _scanLocal(envSet = false, sysProxy = null, nativeIP = null, Hades = console, envData = null) {
    ...
    const [procActive, nyxProxies] = await Promise.all([scanProc(), scanNyx()]);
...
class ProteusMatrix {
  ...
  static _ensureInit() {
    ...
    this._evalPipeline = ["envSet", "nativeCN", "overseasGlobal", "v6Turbo", "idleAgent", "overseasRegion", "biz", "ruleSplit", "needsAirlock", "fallback"];
...
class MBTQuoCRS {
  ...
  constructor(logger, Rid, logTag, colorCode, parentSignal = null, cerSessionId = null) {
    ...
    this._onParentAbort = () => {
      this.logger.warn(`${this.uiRid} 收到上游中断信号正在终止任务...`);
      this.stop();
    };
  ...
  addTask(id, factory, BPP = false, delay = 0) {
    ...
    this.logger.debug(`${this.uiRid} | [Quo] 任务节点 ${id} 将在 ${(delay / 1000).toFixed(1)}s 后挂载...`);
  ...
  _activate(id, factory, BPP) {
    ...
    const task = {
      id, name, BPP, context, start: now,
      curr: 0, prev: 0, speed: 0,
      active: now, lastUpdate: now,
      bytesTotal: 0, byteIdleMs: 0, bytes: 0,
      state: MBTQuoCRS.Task_State.Running
    };

    this.tasks.set(id, task);
    const roleText = BPP ? " (备用)" : "";
    this.logger.debug(`${this.uiRid} | [Quo] 节点挂载: [${name}]${roleText}`);
    promise.then(
  ...
  _monitor() {
    ...
    if (this._cerSessionId) {
      ...
      const _anyStage = Array.from(this.tasks.values()).some((t) => {
        ...
        const p = t.context?.telemetry?.progress ?? 0;
        return s === "compressing" || s === "resolving" || (s === "receiving" && p > 15);
      });
      const maxByteIdle = allHandshaking ? 180000 : _maxProg > 90 ? 240000 : _maxProg > 75 ? 180000 : _maxProg > 50 ? 135000 : _maxProg > 25 ? 120000 : _anyStage ? 180000 : 90000;
      const guardErr = cerberus.guard(this._cerSessionId, { maxPulseIdle: 90000, maxByteIdle, grace: baseGrace });
      if (guardErr) {
        this.logger.warn(`${this.uiRid} | [Quo] 触发: ${guardErr.message}，强制终止所有任务`);
    ...
    for (const task of snapshot) {
      const taskRuntime = now - task.start;
      if (taskRuntime > 30 * 60 * 1000) {
        const _tt = task.context?.telemetry;
        const _ts = _tt?.ewma_speed ?? 0;
        const _tActive = (now - (task.active || task.start)) < 60000;
        if (_ts > 100 * 1024 && _tActive && taskRuntime < 50 * 60 * 1000) {
        } else {
          this.logger.warn(`${this.uiRid} | [Quo] 任务 [${task.name}] 超过最大运行时限`);
      ...
      if (!leader) {
        ...
        const lBytes = leader.io_bytes || leader.bytes || 0;
        const tBytes = task.io_bytes || task.bytes || 0;
        if (Math.abs(tBytes - lBytes) > 1024) {
          if (tBytes > lBytes) leader = task;
        } else if (task.curr > leader.curr) {
          leader = task;
        }
      }
    }
    ...
    this._leaderStatus = {
      count: this.tasks.size + this.pendingTimers.size,
  ...
  _judge(task, leader, now) {
    ...
    const tObj = t.git_objects || 0;
    const stage = t.git_stage || "";
    const isLateStage = stage === "resolving" || stage === "checkout";
    const grace = tObj > 10000 ? 90000 : tObj > 1000 ? 60000 : 45000;
    if (runtime < grace) {
      if (task.curr > 0 || runtime <= 15000 || t.connection_state === "HANDSHAKING") return;
      if (!fresh && runtime < grace + 15000) return;
    ...
    let lagTh = 30;
    if (tObj > 10000) lagTh = 15;
    else if (tObj > 1000) lagTh = 20;
    if (isLateStage) lagTh = Math.max(lagTh, 40);

    if (leader && task.id !== leader.id && leader.curr > task.curr + lagTh) {
      ...
      if (lSpeed >= 512 * 1024 && pulse) {
        ...
        if (leader.curr - task.curr <= lagTh + 20 * ratio) return;
      }
    ...
    if (leader?.speed > 2 && task.speed < 0.2) {
      ...
      if (task.curr > 50 && now - task.active < 120000) {
        return;
      }
      if (task.curr > 75 && now - task.active < 180000) {
        return;
      }
    ...
    const checkout100Timeout = tObj > 10000 ? 180000 : 120000;
    if (leader && task.id === leader.id && task.curr >= 99 && now - task.active > checkout100Timeout) {
  ...
  _fail(id, err) {
    ...
    if (!isAborted) {
      ...
      const errType = err.code || err.message?.split("\n")[0]?.slice(0, 60) || "UNKNOWN";
      PoseidonSpear.recordResult(task.name, false, 0, errType);
    } else {
  ...
  getStatus() {
    ...
    for (const task of this.tasks.values()) {
      ...
      if (now - task.active < 60000) activeCount++;
      leaderBytes = Math.max(leaderBytes, task.bytes || 0);
    }
    return { count: this.tasks.size + this.pendingTimers.size, maxProgress, activeCount, leaderBytes };
  }
...
class MBTSignalTrap extends EventEmitter {
  ...
  async _emitAdvance(newGen) {
    ...
    await Promise.allSettled(
      listeners.map((fn) => {
        try {
          return Promise.resolve(fn(newGen));
        } catch (e) {
...
const Git_Log_Whitelist =
  "fatal:|error:|remote:|warning:|Could not resolve|timed out|Connection refused|Connection was reset|Recv failure|SSL|certificate|HTTP/2|stream|ESLOWNET|E_GIT_IO_STALL|E_GIT_BYTE_IDLE_TIMEOUT|E_GIT_ZOMBIE_IDLE|E_GIT_SPEED_FLOOR|龟速|假死|early EOF|index-pack|unpack-objects|write error|No space left|out of memory|memory exhausted|repository not found|Authentication failed|unexpected disconnect|RPC failed|curl|flush after ref|bad/illegal format|missing URL|unable to access|unable to update".split(
    "|"
  );

const Git_Diag128_Patterns = [
...
const Commit_Game_Prefixes = [
  ["原神", "gs"],
  ["星铁", "sr"],
  ["绝区零", "zzz"],
  ["鸣潮", "waves"]
].map(([name, key]) => ({
  pattern: new RegExp(`^(${name}UP[:：])\s*`, "i"),
  key
}));
...
const Help_Fallback_Lines = [
  ...
  "  #咕咕牛设置净化等级[0-2]: 过滤敏感图",
  "  #咕咕牛面板: 查看设置",
  "  #咕咕牛设置[xx][开启/关闭]: Ai/彩蛋/横屏/原图拦截/官方立绘",
  "--------------------"
];

class PoseidonSpear {
  static STRATEGIES = Git_Strike_Strategies;
  static H2_ERRORS = Git_H2_Errors;
  static LOG_WHITELIST = Git_Log_Whitelist;
  ...
  static get _stats() {
    const Stat_Symbol = Symbol.for("Yz.CowCoo.MBT.PoseidonSpear.Stats.v1");
    const wrapper = global[Stat_Symbol];
    if (!wrapper) {
      const stats = new Map();
      Moirai.stamp(Stat_Symbol, stats);
      return stats;
    }
    return wrapper.value ?? wrapper;
  }

  static recordResult(nodeName, success, speed = 0, errorType = null) {
    if (!nodeName) return;
    const stats = this._stats;
    let ns = stats.get(nodeName);
    if (!ns) {
      ns = { sc: 0, fc: 0, spd: 0, errs: {}, ts: Date.now() };
      stats.set(nodeName, ns);
    }
    const alpha = 0.3;
    if (success) {
      ns.sc++;
      ns.spd = ns.spd > 0 ? Math.round(alpha * speed + (1 - alpha) * ns.spd) : Math.round(speed);
    } else {
      ns.fc++;
      if (errorType) {
        ns.errs[errorType] = (ns.errs[errorType] || 0) + 1;
        const sorted = Object.entries(ns.errs).sort((a, b) => b[1] - a[1]);
        ns.errs = Object.fromEntries(sorted.slice(0, 3));
      }
    }
    ns.ts = Date.now();
  }

  static getSuccessRate(nodeName) {
    const ns = this._stats.get(nodeName);
    if (!ns) return 0.5;
    const total = ns.sc + ns.fc;
    return total > 0 ? ns.sc / total : 0.5;
  }

  static getAvgSpeed(nodeName) {
    const ns = this._stats.get(nodeName);
    return ns?.spd || 0;
  }

  static getNodeScore(nodeName) {
    const rate = this.getSuccessRate(nodeName);
    const spd = this.getAvgSpeed(nodeName);
    const liveBonus = this.isLive(nodeName) ? 1 : 0;
    return liveBonus * (0.5 * rate + 0.5 * Math.min(1, spd / (2 * 1024 * 1024)));
  }
  ...
  static diagnose128(rawLog) {
    ...
    for (const p of Git_Diag128_Patterns) {
...
class Cerberus {
  ...
  pulse(sessionId, patch = {}) {
    ...
    if (patch.localKilled) sess.localKilled = true;
  }

  guard(sessionId, options = {}) {
    const sess = this.#sessions.get(sessionId);
    if (!sess || sess.state !== "running") return null;
    if (sess.localKilled) return null;
    const now = Date.now();
...
const DFC = {
  ...
  Ass2_Github_URL: "https://github.com/GuGuNiu/Miao-Plugin-MBT-3/",
  Ass3_Github_URL: "https://github.com/GuGuNiu/Miao-Plugin-MBT-4/",
...
const Nomos_Universe_Rows = [
  ...
  [
    ...
    "sr-character",
    "miao-plugin",
    ["fire", "ice", "wind", "elec", "phy", "quantum", "imaginary"],
    { fire: "火", ice: "冰", wind: "风", elec: "雷", phy: "物理", quantum: "量子", imaginary: "虚数" }
  ],
  ...
  [
    ...
    "waves-character",
    "waves-plugin",
    ["rerong", "lengning", "daodian", "qidong", "yanshe", "yanmie"],
    { 冷凝: "lengning", 热熔: "rerong", 导电: "daodian", 气动: "qidong", 衍射: "yanshe", 湮灭: "yanmie" }
  ]
];
...
function MBTPipeControl(command, args, options = {}, timeout = 0, onStdErr, onStdOut, onProgress, onSlowSpeed, MBTProcc) {
  ...
  const traceId = options.traceId ?? `${Rid}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 8)}`;
  ...
  const stripProxyVars = (env, keep = null) =>
    proxyVars.forEach((v) => {
      if (!keep?.has(v)) delete env[v];
    });
  ...
  if (options.airlock) {
    ...
    stripProxyVars(runEnv, explicitProxyKeys);
  ...
  const _CaAudit = async () => {
    ...
    if (Array.isArray(options.caWhitelist) && options.caWhitelist.length > 0) {
      ...
      let targetHost = null;
      try {
        targetHost = new URL(normalizedTarget).host;
      } catch {}
    ...
    if (MiaoPluginMBT?.MetaMutex?.run) {
      await MiaoPluginMBT.MetaMutex.run(
        async () => {
          if (checkLocal()) return;
  ...
  return new Promise(async (resolve, reject) => {
    ...
    let hardDeadline = 0;
    let maxHardDeadline = 0;
    let lastExtCheck = 0;
    let lastExtProgress = 0;
    ...
    const killProcess = async (code, reason, force = false) => {
      ...
      const cerId = options.cerSessionId;
      if (cerId) {
        try {
          Cerberus.getInstance().pulse(cerId, { localKilled: true, event: "local-kill", message: reason });
        } catch {}
      }
      ...
      const _killer = MBTProcc?.kill ? (sig) => MBTProcc.kill(proc, sig) : (sig) => MBTProcPool.kill(proc, sig);
    ...
    if (isClone) {
      hardDeadline = Date.now() + constraints.hardTimeout;
      maxHardDeadline = hardDeadline + constraints.hardTimeout;
      lastExtCheck = Date.now();
    }

    const emitTelemetry = (immediate = false) => {
      const now = Date.now();
      if (!immediate && now - lastTelemetryEmit < 400) return;
      lastTelemetryEmit = now;
      try {
        options.onTelemetry?.({ ...telemetry, pid: proc?.pid });
      } catch {}
    };

    const emitProgress = (payload) => {
      try {
        onProgress?.({ ...payload, traceId });
      } catch {}
    };
    ...
    Pulse = setInterval(() => {
      ...
      if (isGitTransfer && telemetry.io_bytes === 0 && now - telemetry.startTime > 180000) {
        killProcess("E_Handshake_Timeout", `握手绝对超时(${Math.floor((now - telemetry.startTime) / 1000)}s)`);
        return;
      }

      if (telemetry.io_bytes > 0 && now - lastActiveTime > constraints.stallThreshold) {
      ...
      if (isGitTransfer && intervalBytes > 0 && (
        ...
        (telemetry.git_stage === "receiving" && lastPercent > 15)
      )) {
        lastBytePulse = now;
      }
      ...
      if (isClone && hardDeadline > 0) {
        if (now - lastExtCheck >= 30000) {
          lastExtCheck = now;
          if (telemetry.ewma_speed > 100 * 1024 && lastPercent > lastExtProgress) {
            lastExtProgress = lastPercent;
            hardDeadline = Math.min(hardDeadline + 60000, maxHardDeadline);
          }
        }
        if (now > hardDeadline) {
          killProcess("ETIMEDOUT", `硬性超时: 任务运行超过 ${Math.floor((now - telemetry.startTime) / 60000)} 分钟`);
          return;
        }
      }
    }, 1000);
    ...
    if (timeout > 0 && !isClone) {
      timer = setTimeout(() => {
        killProcess("ETIMEDOUT", `命令执行超时 (${timeout}ms)`);
      }, timeout);
    }

    const lineBuf = { stdout: "", stderr: "" };
    const StageWeight = { counting: [0, 5], compressing: [5, 15], receiving: [15, 85], resolving: [85, 95], checkout: [95, 100] };
    ...
    const OutStream = (streamName, data, externalCallback) => {
      ...
      const combined = lineBuf[streamName] + chunk;
      const parts = combined.split(/[\r\n]/);
      lineBuf[streamName] = parts.pop() || "";
      ...
      for (const line of parts) {
        ...
        if (streamName === "stdout") {
          ...
          if (stderrLen < Constraints.MAX_BUFFER) {
            ...
            stderrChunks.push(line + "\n");
            stderrLen += line.length + 1;
            while (stderrLen > Constraints.MAX_BUFFER && stderrChunks.length > 2) {
              const removed = stderrChunks.shift();
              stderrLen -= removed.length;
            }
          }
          ...
          if (line.includes("Receiving objects:") || line.includes("Unpacking")) {
            telemetry.connection_state = "TRANSFERRING";
...
let ExportMod = null;
let CreFaceMod = null;
let CommunityMBT = null;
let CommunityRules = [];
let FetcherMBT = null;
let FetcherRules = [];
let MBTMapTileAss = null;

const _DocHubStub = {
  async report() {},
  _diagnose() {
    return { summary: "", contextInfo: "", suggestions: "", stack: "" };
  },
  async _consultOracle() {
    return "云露分析服务暂时不可用。";
  },
  _extractErrSource() {
    return null;
...
const _CreFaceStub = {
  ResolveFace: async () => null,
  FindZZZIcon: async () => null,
  ResolveWavesFace: () => null,
  ResolveCREFace: async () => null
};
const _MapTileStub = {
  render: async () => null
};

const modPath = (category, file) =>
  Nomos.isDeployed
    ? path.join(MiaoPluginMBT.Paths.OpsPath, "modules", category, file)
    : path.join(Nomos.ResPoolDir, "modules", category, file);
const getFactory = (mod, name) => mod?.[name] ?? mod?.default?.[name];

class HotModule {
  static #entries = new Map();
  static #fallbacks = new Map();

  static register({ name, label, resolvePath, factory, fallback, workerData }) {
    this.#entries.set(name, {
      label: label || name, resolvePath, factory,
      workerData, instance: null, path: null,
      gen: 0, moiraiGen: 0, hestiaToken: null
    });
    fallback && this.#fallbacks.set(name, fallback);
  }

  static registerAll(specs) {
    for (const s of specs) s?.name && this.register(s);
  }

  static async #resolve(spec) {
    if (!spec.resolvePath) return null;
    const cands = typeof spec.resolvePath === "function" ? spec.resolvePath() : spec.resolvePath;
  ...
  static #cleanupInstance(spec) {
    if (spec.hestiaToken) {
      Hestia.forceCleanup(spec.hestiaToken);
      spec.hestiaToken = null;
    } else if (typeof spec.instance?.terminate === "function")
      try {
        spec.instance.terminate();
      } catch {}
  }
  ...
  static #commit(spec, inst, modulePath, mtime, gen) {
    spec.instance = inst;
  ...
  static async load(name, ctx = {}) {
    const spec = this.#entries.get(name);
    if (!spec) throw new Error(`未注册模块 "${name}"`);
    const curGen = Moirai.currentGen;
    const modulePath = spec.resolvePath ? await this.#resolve(spec) : null;
    let mtime = 0;
    if (modulePath)
      try {
        mtime = fs.statSync(modulePath).mtimeMs;
      } catch {}

    if (spec.instance && spec.path === modulePath && spec.mtime === mtime && spec.moiraiGen === curGen) return spec.instance;

    this.#cleanupInstance(spec);

    if (spec.factory) {
      const inst = spec.factory({ ...ctx, modulePath });
      this.#commit(spec, inst, modulePath, mtime, curGen);
      if (typeof inst?.terminate === "function") {
        const token = `HotModule_${name}_${curGen}_${Date.now().toString(36)}`;
        spec.hestiaToken = token;
        Hestia.register("HotModule", token, inst, () => {
          try {
            inst.terminate();
          } catch {}
        });
    ...
    if (!modulePath) {
      Hades.D(`"${spec.label}" 模块未部署`);
      return null;
    }
    const mod = await import(`${toFileUrl(modulePath)}?v=${curGen}_${Math.floor(mtime)}`);
    const inst = mod.default || mod;
    this.#commit(spec, inst, modulePath, mtime, curGen);
    return inst;
  }

  static async invoke(name, method, ...args) {
    if (!this.#entries.has(name)) throw new Error(`未注册模块 "${name}"`);
    try {
      const inst = await this.load(name);
      return await (typeof inst?.[method] === "function"
        ? inst[method](...args)
        : typeof inst === "function"
          ? inst(method, ...args)
          : Promise.reject(new Error(`模块 "${name}" 不支持方法 "${method}"`)));
    } catch (err) {
      const fb = this.#fallbacks.get(name);
      return fb && typeof fb[method] === "function" ? fb[method](...args) : Promise.reject(err);
    }
  }
  ...
  static snapshot() {
    const snap = {};
    for (const [name, spec] of this.#entries) snap[name] = { loaded: !!spec.instance, path: spec.path, gen: spec.gen, moiraiGen: spec.moiraiGen };
    return snap;
  }
...
async function DocHubModule() {
  try {
    const mod = await HotModule.load("dochub");
    if (!mod) {
      DocHub = _DocHubStub;
      ErrDoc = _ErrDocStub;
      return;
    }
    const fn = getFactory(mod, "DocHub");
    const result = fn?.({ Ananke, Nomos, HadesEntry, getCore, getHades, MiaoPluginMBT, Morpheus, Pheme, Version, HotModule });
    if (result) {
      DocHub = result.DocHub;
      ErrDoc = result.ErrDoc;
    }
  } catch (err) {
    Hades.E("错误报告模块加载失败:", err);
    DocHub = _DocHubStub;
    ErrDoc = _ErrDocStub;
  }
}

async function DockerModule() {
  try {
    const fn = getFactory(await HotModule.load("docker"), "Docker");
    if (fn) DockerMod = fn({});
  } catch (err) {
    Hades.E("Docker检测模块加载失败:", err);
  }
}

async function ProtocolModule() {
  try {
    const fn = getFactory(await HotModule.load("protocol"), "Protocol");
    if (fn) ProtocolMod = fn({ DockerMod });
  } catch (err) {
    Hades.E("协议适配模块加载失败:", err);
  }
}

async function MasterMsgModule() {
  try {
    const fn = getFactory(await HotModule.load("mastermsg"), "MasterMsg");
    if (fn) MasterMsg = fn({ common, yaml, getYzPath: () => MiaoPluginMBT.Paths.YzPath });
  } catch (err) {
    Hades.E("通知主人模块加载失败:", err);
  }
}

async function ExportModule() {
  try {
    const fn = getFactory(await HotModule.load("export"), "Export");
    if (fn) ExportMod = fn({ Ananke, Pheme, MiaoPluginMBT, Tianshu, getHades, DocHub, toPosix, common, segment });
  } catch (err) {
    Hades.E("图片导出模块加载失败:", err);
  }
}

async function CreFaceModule() {
  try {
    const fn = getFactory(await HotModule.load("creface"), "CreFace");
    CreFaceMod = fn ? fn({ MiaoPluginMBT, Nomos, Ananke, toFileUrl }) : _CreFaceStub;
  } catch (err) {
    Hades.E("角色头像模块加载失败:", err);
    CreFaceMod = _CreFaceStub;
  }
}

async function MapTileModule() {
  try {
    const fn = getFactory(await HotModule.load("maptile"), "MapTile");
    MBTMapTileAss = fn ? fn({ getHades, Tianshu, Cerberus, MBTSignalTrap, Morpheus, Nomos, MiaoPluginMBT, Ananke }) : _MapTileStub;
  } catch (err) {
    Hades.E("图库游戏地图模块加载失败:", err);
    MBTMapTileAss = _MapTileStub;
  }
}

async function CommunityModule() {
  try {
    const fn = getFactory(await HotModule.load("ops"), "Community");
    const result = fn?.({ 
      MiaoPluginMBT, Ananke, Metis, Tianshu, Morpheus, DocHub, MBTPipeControl, 
      DFC, YzPath, Pheme, HadesEntry, getCore, getHades, MBTProcPool, Cerberus, common, 
      Proteus, Hermes, PoseidonSpear, RuntimeCtx 
    });
    if (result) {
      CommunityMBT = result.PluginClass;
      CommunityRules = result.rules;
    }
  } catch (err) {
    Hades.E("社区图库模块加载失败:", err);
  }
}

async function FetcherModule() {
  try {
    const fn = getFactory(await HotModule.load("queues"), "Fetcher");
    const result = fn?.({ 
      MiaoPluginMBT, Ananke, Metis, Morpheus, DocHub, MBTPipeControl, DFC, 
      YzPath, Pheme, HadesEntry, getCore, getHades, MBTProcPool, Cerberus, common, Hermes 
    });
    if (result) {
      FetcherMBT = result.PluginClass;
      FetcherRules = result.rules;
    }
  } catch (err) {
    Hades.E("下载器模块加载失败:", err);
  }
}

async function SentinelModule() {
  try {
    const fn = getFactory(await HotModule.load("sentinel"), "Sentinel");
    if (fn) Sentinel = fn({ 
      HotModule, DocHub, ErrDoc, Ananke, Nomos, HadesEntry, getCore, 
      getHades, MiaoPluginMBT, Pheme, CommunityMBT 
    });
  } catch (err) {
    Hades.E("哨兵监控模块加载失败:", err);
  }
}

async function reloadModules() {
  await HotModule.reload("docker");
  await HotModule.reload("protocol");
  await HotModule.reload("agents");
  await HotModule.reload("dochub");
  await HotModule.reload("mastermsg");
  await HotModule.reload("export");
  await HotModule.reload("creface");
  await HotModule.reload("maptile");
  await HotModule.reload("ops");
  // await HotModule.reload("queues");
  await HotModule.reload("sentinel");
  return true;
}
...
HotModule.registerAll([
  { name: "docker", label: "Docker", resolvePath: () => modPath("adapter", "docker.js") },
  { name: "protocol", label: "协议适配", resolvePath: () => modPath("adapter", "protocol.js") },
  {
    name: "worker",
    label: "Worker线程",
    resolvePath: () => modPath("infra", "worker.js"),
    fallback: _wFallback,
    factory: _wFactory
  },
  { name: "ops", label: "社区图库", resolvePath: () => modPath("community", "index.js") },
  { name: "ops-reload", label: "模块重载", resolvePath: () => null, factory: () => ({ reload: reloadModules }) },
  { name: "agents", label: "智能体诊断", resolvePath: () => modPath("agents", "agents.js") },
  { name: "dochub", label: "错误报告", resolvePath: () => modPath("agents/diagnostic", "dochub.js") },
  { name: "sentinel", label: "哨兵监控", resolvePath: () => modPath("agents", "sentinel.js") },
  { name: "mastermsg", label: "通知主人", resolvePath: () => modPath("feature", "mastermsg.js") },
  { name: "export", label: "图片导出", resolvePath: () => modPath("feature", "export.js") },
  { name: "creface", label: "角色头像解析", resolvePath: () => modPath("feature", "creface.js") },
  { name: "maptile", label: "图库游戏地图", resolvePath: () => modPath("feature", "maptile.js") },
  { name: "queues", label: "通用下载器", resolvePath: () => modPath("queue", "queues.js") }
]);
...
class Morpheus {
  ...
  static #FindBrowserPath() {
    const platform = os.platform();
    if (platform === "win32") {
      const suffixes = [path.join("Microsoft", "Edge", "Application", "msedge.exe"), path.join("Google", "Chrome", "Application", "chrome.exe")];
    ...
    if (platform === "linux") {
      ...
      const linuxPaths = ["/usr/bin/chromium-browser", "/usr/bin/chromium", "/usr/bin/google-chrome-stable", "/usr/bin/google-chrome", "/usr/bin/chrome", "/snap/bin/chromium"];
  ...
  static #isBrowserHealthy() {
    try {
      return this.#browserInstance && typeof this.#browserInstance === "object" && this.#browserInstance.connected === true;
    } catch {
      return false;
    }
  }
  ...
  static async #getBrowser(logger = console) {
    ...
    if (this.#browserInitLock) {
      let waited = 0;
      while (this.#browserInitLock) {
  ...
  static async shot(businessName, options = {}) {
    ...
    if (renderLock?.run && !options.__renderLocked) {
      return await renderLock.run(() => this.shot(businessName, { ...options, __renderLocked: true }), {
        id: `Morpheus_lock:${businessName}:${Date.now()}`,
        ttl: 120000,
        wait: 0
      });
    }
    ...
    const {
      ...
      navOpts = { waitUntil: "load", timeout: 30000 },
      pageBoundingRect,
      width,
      padding = 0 /*20*/,
    ...
    const DataMaps = {
      ...
      Cow_Res_Path: toFileUrl(`${MiaoPluginMBT.Paths.OpsPath}/resources/`),
      ResPool_Res_Path: await Nomos.getCRPRootPath(Hades),
    ...
    try {
      ...
      await page.goto(toFileUrl(htmlPath), { waitUntil: "load", timeout: 30000, ...navOpts });

      if (MorpheusSignal && signalPromise) {
        const signalResult = await new Promise((resolve) => {
          signalPromise.then(() => resolve(true));
          setTimeout(() => resolve(false), 60000);
        });
        if (!signalResult) {
          Hades.W(`[${businessName}] 渲染信号超时`);
        }
      } else {
        await page
          .waitForFunction(
        ...
        try {
          await Promise.race([page.evaluate(() => document.fonts.ready), new Promise((_, reject) => setTimeout(() => reject(new Error("font wait timeout")), 15000))]);
        } catch (e) {}
      }
  ...
  static #resolveImgBase() {
    if (Nomos.isDeployed) {
      return path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "img");
    }
    return path.join(Nomos.ResPoolDir, "resources", "img");
  }

  static async pickBg() {
    const imgBase = this.#resolveImgBase();
    const files = await this.#scanDir(this.#bgCache, "bg", imgBase);
    const selected = MBTMath.Sample(files);
    return selected ? toFileUrl(path.join(imgBase, "bg", selected)) : "";
  }

  static async pickHeader() {
    const imgBase = this.#resolveImgBase();
    const files = await this.#scanDir(this.#headerCache, "picture", imgBase);
    const selected = MBTMath.Sample(files);
    return selected ? toFileUrl(path.join(imgBase, "picture", selected)) : "";
  }
  ...
  static async pickHeaderSet(count = 20) {
    ...
    const base = path.join(imgBase, "picture");
  ...
  static getStaticImg(filename, subDir = "") {
    const targetPath = subDir ? path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "img", subDir, filename) : path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "img", filename);
    return toFileUrl(targetPath);
  }

  static async #scanDir(cacheObj, subDirName, imgBase = null) {
    const now = Date.now();
    if (now - cacheObj.lastScan < cacheObj.ttl && cacheObj.files.length > 0) return cacheObj.files;
    const base = imgBase || path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "img");
    const targetDir = path.join(base, subDirName);
    try {
      const entries = await Ananke.readDir(targetDir);
      const newFiles = entries.filter((e) => e.isFile() && /\.(webp|png|jpg|jpeg)$/i.test(e.name)).map((e) => e.name);
...
class Ananke {
    ...
    if (typeof target === "string") {
      ...
      const processDir = async (dir) => {
        try {
          ...
          const sizes = await this.parallel(
            files,
            async (f) => {
              try {
                return (await fsPromises.stat(f)).size;
              } catch {
                return 0;
              }
            },
  ...
  static #FilterCfg(data) {
    const allowedKeys = ["Repo_Ops", "PFL_Ops", "RenderScale", "Ai", "EasterEgg", "layout", "SR18_Ops", "Preempt", "MihoyoSplash"];
    return Object.fromEntries(allowedKeys.filter((k) => Object.hasOwn(data, k) || Object.hasOwn(DFC ?? {}, k)).map((k) => [k, data[k] ?? DFC[k]]));
  }
...
class Nomos {
  ...
  static PurgeTargets(pathsMap) {
    ...
    const commonPaths = [pathsMap.ComResPath, pathsMap.GsPath, pathsMap.ProvisionPath, pathsMap.TempHtmlPath].filter(Boolean);
  ...
  static AllRepoPaths(pathsMap) {
    return this.#REGISTRY.map((r) => pathsMap[r.pathKey]).filter(Boolean);
  }
  ...
  static async ModuleOps(repoPath, gameKey, action) {
    ...
    const shouldEnable = action === "enable" ? true : action === "disable" ? false : !!(await this.getContext())?.[`${gameKey}Installed`];

    const ok = shouldEnable ? await this.PackOps(repoPath, [normalized], null) : await this.PackOps(repoPath, null, [normalized]);

    if (!ok) Hades.D(`可选游戏管理失败: ${action || "check"} ${gameKey} @ ${repoPath}`);
    return ok;
  }
  ...
  static get Universe() {
    return Object.fromEntries(
      Nomos_Universe_Rows.map(([key, name, dirName, plugin, sortOrders, elemMap]) => [
        key, { name, key, dirName, sortOrders, elemMap, pluginPath: path.join(YzPath, "plugins", plugin) }
      ])
    );
  }
  ...
  static async getTemplate(filename) {
    ...
    let basePath;
    if (this.isDeployed) {
      basePath = MiaoPluginMBT.Paths.OpsPath;
    } else {
      basePath = (await this.ensureCRP({ useLocal: true, logger: console })) || this.ResPoolDir;
    }
    const localPath = path.join(basePath, "resources", "html", subDir, filename);
  ...
  static async CleanDataPool(type, Hades = null, options = {}) {
    ...
    if (Array.isArray(cached) && cached.length > 0) {
      return randomUA ? cached[MBTMath.Range(0, cached.length - 1)].replace(/(\d+)\.0/, `$1.${MBTMath.Range(1, 100)}`) : cached;
    }

    const raw = await this.getCRPData(config.path, Hades || console);
    if (!raw) {
      if (!randomUA) H.D(`前置库暂不可用: ${config.path}`);
    ...
    if (Array.isArray(parsed) && parsed.length > 0) {
      const cleaned = parsed.map((v) => (typeof v === "string" ? v.trim().replace(/^`+|`+$/g, "") : v)).filter((v) => !!v);
      const deduped = Array.from(new Set(cleaned));
      this.#_cacheSet(this.#_poolCache, config.cacheKey, deduped, this.#_POOL_TTL);
      return randomUA ? deduped[MBTMath.Range(0, deduped.length - 1)].replace(/(\d+)\.0/, `$1.${MBTMath.Range(1, 100)}`) : deduped;
    }
  ...
  static async getOracleConfig(Hades = null) {
    ...
    const raw = await this.getCRPData("prompt", Hades || console);
  ...
  static async getDiagnosisConfig(Hades = null) {
    const cacheKey = "Config:Diagnosis";
    const cached = this.#_cacheGet(this.#_configCache, cacheKey);
    if (cached) return JSON.parse(cached);

    const raw = await this.getCRPData("diagnosis", Hades || console);
    if (raw) {
      try {
        const config = JSON.parse(raw);
        this.#_cacheSet(this.#_configCache, cacheKey, raw, this.#_CONFIG_TTL);
  ...
  static #emitProgress(onProgress, stage, percent, detail = "") {
    if (typeof onProgress === "function") {
      try {
        onProgress({ stage, progress: percent, detail, timestamp: Date.now() });
      } catch {}
    }
  }
  ...
  static get isDeployed() {
    return fs.existsSync(path.join(MiaoPluginMBT.Paths.OpsPath, "modules"));
  }
  ...
  static async ensureCRP(options = {}) {
  ...
  static #File_Map = {
    prompt: "modules/agents/rule/prompt.json",
    diagnosis: "modules/agents/diagnostic/diagnosis.json",
  ...
  static async readResPoolData(relativePath, options = {}) {
    ...
    try {
      if (!this.isDeployed) {
        const repoDir = await this.ensureCRP({ force: false, useLocal: true, logger, busyWaitMs });
        if (!repoDir) return null;
        const filePath = path.join(repoDir, relativePath);
        if (!fs.existsSync(filePath)) return null;
        return Ananke.readFile(filePath, encoding);
      }
      const localPath = path.join(MiaoPluginMBT.Paths.OpsPath, relativePath);
      if (!fs.existsSync(localPath)) return null;
      return Ananke.readFile(localPath, encoding);
    } catch (err) {
      return null;
    }
  }
  ...
  static async getCRPData(key, logger) {
    return this.readResPoolData(this.getResPoolMapPath(key), { logger });
  }

  static async getCRPRootPath(logger = console) {
    try {
      if (this.isDeployed) {
        return toFileUrl(path.join(MiaoPluginMBT.Paths.OpsPath, "resources") + "/");
      }
      const result = await this.ensureCRP({ useLocal: true, logger });
      if (result) return toFileUrl(path.join(result, "resources") + "/");
      return "";
    } catch {
      return "";
    }
  ...
  static #queueResPool(options = {}) {
    ...
    const job = Promise.race([this.syncResPool({ force, logger, onProgress }), new Promise((_, reject) => setTimeout(() => reject(new Error("前置库同步超时")), timeout))]).finally(
      () => {
  ...
  static async syncResPool(options = {}) {
    ...
    const doClone = async (signal) => {
      ...
      await MBTPipeControl(
        "git",
        ["clone", "--template=", "--depth=1", "--single-branch", "-b", "master", this.#_crppRepoUrl, targetDir],
        { cwd: path.dirname(targetDir), Rid: "CRP", caDisabled: true, signal, gitConfigs: ["gc.auto=0"] },
        60000, null, cloneProgress, null, null, null
      );
          ...
          async (signal) => {
            ...
            const clearLocks = () => {
              ["index.lock", "shallow.lock", "HEAD.lock"].forEach((lock) => {
                ...
                try {
                  if (fs.existsSync(lp)) fs.rmSync(lp, { force: true, maxRetries: 5, retryDelay: 200 });
                } catch {}
              });
            };
            clearLocks();
    ...
    try {
      await this.ResPoolLock.run(
        async (signal) => {
          ...
          const clrLock = () =>
            [lockPath, path.join(gitDir, "HEAD.lock")].every((lp) => {
              try {
                if (fs.existsSync(lp)) fs.rmSync(lp, { force: true, maxRetries: 3, retryDelay: 100 });
              } catch {}
          ...
          const syncByReset = async () => {
            ...
            await MBTPipeControl("git", ["fetch", "origin"], { cwd: targetDir, Rid: "CRP", caDisabled: true, signal, gitConfigs: ["gc.auto=0"] }, 30_000);
            this.#emitProgress(onProgress, "update_reset", 60, "重置到最新版本");
            await MBTPipeControl("git", ["reset", "--hard", "origin/master"], { cwd: targetDir, Rid: "CRP", caDisabled: true, signal, gitConfigs: ["gc.auto=0"] }, 10000);
          };

          try {
            this.#emitProgress(onProgress, "update_pull", 50, "尝试拉取更新");
            await MBTPipeControl("git", ["pull", "origin", "master", "--ff-only"], { cwd: targetDir, Rid: "CRP", caDisabled: true, signal, gitConfigs: ["gc.auto=0"] }, 30_000);
...
class Tianshu {
  static async NormalizeName(inputName, options = {}) {
    ...
    if (this._aliasReverseIndex.size === 0) await this.BuildAliasIndex(MiaoPluginMBT._AliasData);
  ...
  static _dpCost(a, b) {
    ...
    for (let i = 1; i <= al; i++) {
      ...
      for (let j = 1; j <= bl; j++) {
        ...
        curr[j] = Math.min(prev[j] + 2.7 * wt, curr[j - 1] + 2.7 * wt, prev[j - 1] + sub);
      }
      [prev, curr] = [curr, prev];
    }
    return prev[bl];
  ...
  static _pinyinModule = null;
  static _pinyinLoading = null;

  static async _loadPinyin() {
    if (this._pinyinModule) return this._pinyinModule;
    if (this._pinyinLoading) return this._pinyinLoading;
    this._pinyinLoading = (async () => {
      try {
        const candidates = [
          path.join(MiaoPluginMBT.Paths.OpsPath, "modules", "infra", "pinyin.mjs"),
          MiaoPluginMBT.Paths.PinyinPath
        ];
        let pinyinPath = null;
        for (const p of candidates) {
          if (p && fs.existsSync(p)) { pinyinPath = p; break; }
        }
        if (!pinyinPath) { this._pinyinModule = false; return false; }
        const mod = await import(toFileUrl(pinyinPath));
        this._pinyinModule = mod?.pinyin || false;
        return this._pinyinModule;
      } catch {
        this._pinyinModule = false;
        return false;
      } finally {
        this._pinyinLoading = null;
      }
    })();
    return this._pinyinLoading;
  }
  ...
  static async BuildAliasIndex(aliasData) {
    if (this._aliasReverseIndex.size > 0) return;
    const pinyinFn = await this._loadPinyin();
    const exact = new Map();
    const gmIdx = new Map();
      ...
      for (const [mainName, aliases] of Object.entries(src)) {
        ...
        if (pinyinFn) {
          for (const term of terms) {
            if (/[\u4e00-\u9fff]/.test(term)) {
              try {
                const fullPy = pinyinFn(term, { toneType: "none", type: "array" }).join("").toLowerCase();
                if (fullPy) allTerms.push(fullPy);
                const initials = pinyinFn(term, { pattern: "first" }).replace(/\s/g, "").toLowerCase();
                if (initials) allTerms.push(initials);
              } catch {}
            }
          }
        }
        rows.push({ mn: mainName, terms: allTerms });
  ...
  static ParseID(identifier) {
    ...
    if (match && match[1] && match[2]) {
      ...
      if (mainName) return { mainName: mainName, imgNum: match[2] };
    }
    return null;
  }
  ...
  static async FsQuery(relativePath) {
    ...
    const preferredPath = imgData?.storagebox ? Nomos.PathKey(imgData.storagebox, MiaoPluginMBT.Paths) : null;
    ...
    const ReliefScope = Nomos.ReliefScope(imgData?.storagebox, MiaoPluginMBT.Paths, MiaoPluginMBT.MBTConfig, context);
  ...
  static async UpdateStats(logger = getCore()) {
    ...
    await Ananke.writeText(MiaoPluginMBT.Paths.RTCPath, JSON.stringify(statsCache, null, 2)).catch((err) => Hades.E(`索引写入仓库统计缓存失败:`, err));
  }
  ...
  static GetStrategy(gameKey) {
    ...
    return {
      ...
      resolveIcon: async (charName) => {
        if (gameKey === "gs" || gameKey === "sr") return CreFaceMod.ResolveFace(gameKey, charName);
        if (gameKey === "waves") return CreFaceMod.ResolveWavesFace(charName);
        return null;
      },
      ...
      for (const repo of ScanQueue) {
        ...
        try {
          ...
          for (const dir of dirs) {
            if (dir.isDirectory()) charSet.add(dir.name);
          }
        } catch {}
      }
...
class Presenter {
  static async RenderDashboard(e, stats) {
    ...
    const imgBuffer = await Morpheus.shot("Search", {
      tplFile: path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "html", "tools", "search_helper.html"),
      data: ViewProps,
      logger: Hades,
  ...
  static async _PaginateFM(e, data) {
        ...
        if (forwardList.length > 1) {
          ...
          if (!ok) {
            for (const node of forwardList) {
              const imgSegs = Array.isArray(node) ? node.filter((n) => n?.type === "image") : node?.type === "image" ? [node] : [];
    ...
    if (data.type === "character") {
      ...
      for (let batchNum = 1; batchNum <= pageCount; batchNum++) {
        ...
        const titleFaceUrl = data.gameKey === "zzz" ? await CreFaceMod.FindZZZIcon(primaryName) : null;
        ...
        const makeForwardMsgTitle = titleSeg ? [titleSeg, ` [${primaryName}] 图库详情 (${batchNum}/${pageCount})`] : `[${primaryName}] 图库详情 (${batchNum}/${pageCount})`;
...
class MBTCF {
  ...
  static async ApplyBans() {
    ...
    for (const relPath of this.#activeBans) {
      ...
      if (targetPath) {
        tasks.push(Ananke.obliterate(targetPath).then((success) => ({ path: relPath, success })));
      }
    }
  ...
  static get secTagsCache() {
    return [...this.#secTags];
  }
  static get remoteBanCount() {
...
class MiaoPluginMBT extends plugin {
  ...
  static _indexByCRE = new Map();
  static _indexByTag = new Map();
  static get _SecTagsCache() {
    return MBTCF.secTagsCache;
  }
  static get _userBanSet() {
    return MBTCF.userBanSet;
  }
  static get _activeBanSet() {
    return MBTCF.activeBanSet;
  }
  ...
  static _applyLifecycleState(nextState) {
    ...
    MiaoPluginMBT.BootLock = nextState === MiaoPluginMBT.LifecycleStates.INITIALIZING || nextState === MiaoPluginMBT.LifecycleStates.TEARING_DOWN;
  ...
  static _busyMap = new Map();
  static grabNode(name) {
    this._busyMap.set(name, (this._busyMap.get(name) || 0) + 1);
  }
  static freeNode(name) {
    const c = this._busyMap.get(name) || 0;
    c <= 1 ? this._busyMap.delete(name) : this._busyMap.set(name, c - 1);
  }
  ...
  static ToImgSeg(input, options = {}) {
    if (!input) return null;
    const { audit = false, fallbackText = null } = options;
    if (Buffer.isBuffer(input)) return segment.image(toBase64Url(input));
  ...
  static async ReplyImg(e, input, fallbackText = "", logger = null) {
    ...
    let imgSeg = null;
    try {
      imgSeg = MiaoPluginMBT.ToImgSeg(input);
      ...
      if (fallbackText) {
        try {
          await Pheme.quote(e, `${fallbackText}\n(Code: ${sendErr?.code || "Send_Fail"})`);
        } catch {}
      }
      return false;
    }
  ...
  static async _CtxPrep(logger = console) {
    const Hades = getHades(logger);
    try {
      const raw = await Nomos.getCRPData("proxyNodes", Hades);
      if (!raw) return false;

      let nodes;
      try {
        const p = JSON.parse(raw);
        nodes = Array.isArray(p) ? p : (p?.F2Pool ?? []);
      } catch {
        return false;
      }
      ...
      const uiList = nodes.filter((n) => n?.ClonePrefix).map((node) => (MiaoPluginMBT.#TopoMap.set(node.ClonePrefix.trim().toLowerCase().replace(/\/$/, ""), node.priority), node));
      DFC.F2Pool = uiList;
  ...
  static _CheckCtx(targetUrl) {
    ...
    const match = (p) => {
      ...
      if (!host) return false;
      try {
        return new URL(np).host === host;
      } catch {
        return false;
      }
    };

    return [...this.#TopoMap.keys()].some(match) || DFC?.F2Pool?.some((n) => n?.ClonePrefix && match(n.ClonePrefix)) || false;
  }
  ...
  static get Paths() {
    ...
    this._pathsCache = {
      YzPath: yz,
      CowCooRepoRoot: root,
      MountRepoPath: repoJoin("Miao-Plugin-MBT"),
      GitFilePath: repoJoin("Miao-Plugin-MBT", ".git"),
      MountRepoPath2: repoJoin("Miao-Plugin-MBT-2"),
      GitFilePath2: repoJoin("Miao-Plugin-MBT-2", ".git"),
      MountRepoPath3: repoJoin("Miao-Plugin-MBT-3"),
      GitFilePath3: repoJoin("Miao-Plugin-MBT-3", ".git"),
      MountRepoPath4: repoJoin("Miao-Plugin-MBT-4"),
      GitFilePath4: repoJoin("Miao-Plugin-MBT-4", ".git"),
      MountRepoPath5: repoJoin("Genshin-CR-Repos"),
      GitFilePath5: repoJoin("Genshin-CR-Repos", ".git"),
      MountRepoPath6: repoJoin("StarRail-CR-Repos"),
      ...
      WavesRoleData: opsJoin("resources", "waves", "RoleData.json"),
      ...
      TempHtmlPath: tempJoin("html"),
      TempNiuPath: tempJoin("CowCoo"),
      TempDownloadPath: tempJoin("CowCoo", "Tasks"),
      NetworkDir: tempJoin("CowCoo", "Network"),
      WorkerPath: cowJoin("modules", "infra", "worker.js"),
      PinyinPath: cowJoin("modules", "infra", "pinyin.mjs"),
      Target: {
  ...
  static MetaHub = {
    async AC(syncEx = false) {
      ...
      await Tianshu.BuildAliasIndex(MiaoPluginMBT._AliasData);
    }
  };
  ...
  static _bindLifecycle(bus, logger) {
    ...
    const moduleReloadListener = async () => {
      await reloadModules();
    };
  ...
  static async init(logger = getCore()) {
    ...
    MiaoPluginMBT.#pendingInit = (async () => {
      const initTask = async () => {
        ...
        for (const mutex of [MiaoPluginMBT.MetaMutex, MiaoPluginMBT.GitMutex, MiaoPluginMBT.InstallMutex, MiaoPluginMBT.RenderMutex, MiaoPluginMBT.CleanMutex, Nomos.ResPoolLock]) {
          if (mutex) {
        ...
        try {
          ...
          MiaoPluginMBT.MBTConfig = await Ananke.loadingConfig(MiaoPluginMBT.Paths.ConfigFilePath, defaultConfig, Hades);
          ...
          const [localCacheData] = await Promise.all([MiaoPluginMBT.ImgMetaAC(true, Hades), MiaoPluginMBT.MetaHub.AC(true)]);
  ...
  static async _teardown(isReload = false, logger) {
    ...
    MiaoPluginMBT.#pendingTeardown = (async () => {
      MiaoPluginMBT._transitionState(states.TEARING_DOWN, [states.UNINITIALIZED, states.INITIALIZING, states.READY], isReload ? "reload teardown" : "shutdown teardown");
  ...
  static async OpsGate(e, commandName, options = {}) {
    const cerberus = Cerberus.getInstance();
    if (cerberus.tier === 1) {
    ...
    const { cooldown: customCooldown = null, deferSet = false } = options;
    const cooldownMap = { 3: 5, 2: 15 };
    const cooldownDuration = customCooldown ?? (cooldownMap[cerberus.tier] || 10);
  ...
  static OpsSetCooldown(commandName, userId, duration) {
    if (typeof redis === "undefined" || !userId || !duration) return;
    const redisKey = `CowCoo:${commandName}:${userId}`;
    redis.set(redisKey, "1", { EX: duration }).catch(() => {});
  }

  static OpsClearCooldown(commandName, userId) {
    if (typeof redis === "undefined" || !userId) return;
    const redisKey = `CowCoo:${commandName}:${userId}`;
  ...
  static async TestGitVoice(repoUrl, ClonePrefix, nodeName) {
    ...
    const actualRepoUrl = !ClonePrefix || nodeName === "GitHub" ? `https://github.com/${targetUrp}.git` : `${cleanPrefix}/github.com/${targetUrp}.git`;
  ...
  static async SCD(logger = getCore()) {
    ...
    if (MiaoPluginMBT.MBTConfig.MihoyoSplash) {
  ...
  static async RevertFile(relativePath, logger = getCore()) {
    ...
    try {
      ...
      Hades.E(`${relativePath} 失败:`, copyError);
      return false;
    }
  }

  static RenderMatrix(baseScale = 1) {
    const currentRenderScale = MiaoPluginMBT.MBTConfig && Object.keys(MiaoPluginMBT.MBTConfig).length > 0 ? MiaoPluginMBT.MBTConfig.RenderScale : DFC.RenderScale;
  ...
  static async SmartTaskHeavy(runtimeContext, repoNum, repoUrl, branch, finalLocalPath, e, logger, sortedNodes = [], MBTProcc, signal = null, lockId = null, Cer_SessionId = null, dlConstraints = null) {
    ...
    const repoRealName = cleanRepoUrl
      .split("/")
      .pop()
      .replace(/\.git$/, "");
    ...
    let retryTimer = null;
    let booked = [];
    const UL = dlConstraints?.active ? dlConstraints : null;
    const activeTempPaths = new Set();
    let forceCleanupTimer = null;
    if (UL) {
      forceCleanupTimer = setInterval(async () => {
        if (isShuttingDown) return;
        try {
          const entries = await Ananke.readDir(MiaoPluginMBT.Paths.TempDownloadPath).catch(() => []);
          const now = Date.now();
          for (const entry of entries) {
            if (!entry.isDirectory() || !entry.name.startsWith(`Git-${repoNum}-`)) continue;
            const fullPath = path.join(MiaoPluginMBT.Paths.TempDownloadPath, entry.name);
            if (activeTempPaths.has(fullPath)) continue;
            try {
              const st = await fsPromises.stat(fullPath);
              if (now - st.mtimeMs < UL.forceCleanupMs) continue;
            } catch { continue; }
            await Ananke.obliterate(fullPath, 2, 500).catch(() => {});
            Hades.D(`${RidColored} 强制清理废弃临时文件夹: ${entry.name}`);
          }
          for (const [proc, meta] of MBTProcc.pool) {
            if (meta.state === MBTProcPool.STATE.ACTIVE && now - meta.startTime > UL.forceCleanupMs * 3) {
              const tempPath = [...activeTempPaths].find((p) => p.includes(proc.spawnargs?.join("") || "\0"));
              if (!tempPath) {
                Hades.D(`${RidColored} 强制终止历史进程: PID=${meta.pid}`);
                await MBTProcc.kill(proc, "SIGKILL").catch(() => {});
              }
            }
          }
        } catch {}
    ...
    try {
      ...
      let targetNode = null;
      let preferV6 = enableV6 && Number.isFinite(v6Lat) ? (Number.isFinite(v4Lat) ? v6Lat * v6Bias < v4Lat : true) : enableV6;
      ...
      if (netMode === Proteus.State.V6_TURBO) {
        ...
        MODE = netMode === Proteus.State.NATIVE ? "NATIVE" : netMode === Proteus.State.USER_AGENT || netMode === Proteus.State.IDLE_AGENT ? "USER_PROXY" : "AIRLOCK_PROXY";
      ...
      const validatedMirrors = mirrorNodes.filter((n) => n.protocol === "HTTP" && Number.isFinite(getNodeLatency(n))).sort((a, b) => getNodeLatency(a) - getNodeLatency(b));
      const primaryMirrors = validatedMirrors.slice(0, 2);
      const reserveMirrors = validatedMirrors.slice(2);
      const getStartDelay = (node, index, riskMode, prevDelay = 0) => {
        if (node.retryDelay) return node.retryDelay + MBTMath.Range(0, 500);
        const latency = getNodeLatency(node);
        const v4Threshold = Proteus._setup.thresholdV4;
        let baseDelay = 0;
        if (!Number.isFinite(latency)) baseDelay = MBTMath.Range(4000, 6000);
        else if (latency < 200) baseDelay = 0;
        else if (latency < v4Threshold) baseDelay = Math.round(latency * 2.5);
        else baseDelay = Math.min(Math.round(latency * 1.5), 4000);
        if (index === 0) return baseDelay;
        const minStagger = 2500 + MBTMath.Range(0, 1000);
        const stagger = Math.max(baseDelay, minStagger);
        const delay = prevDelay + stagger;
        if (riskMode) return Math.min(delay, 12000);
        return delay;
      };
      ...
      const createTaskFactory = (node, isDowngrade = false) => {
        return (context, sitRep) => {
          ...
          if (isGithubTarget) actualCloneUrl = cleanRepoUrl;
            ...
            const _urpMatch = cleanRepoUrl.match(/github\.com\/([^/]+\/[^/]+?)(?:\.git)?$/i);
            const _urp = _urpMatch ? _urpMatch[1] : cleanRepoUrl.split("/").pop()?.replace(/\.git$/, "") || cleanRepoUrl;
            actualCloneUrl = `${cleanPrefix}/github.com/${_urp}.git`;
          }
          ...
          const executeGit = async () => {
            ...
            const gitOptions = {
              ...
              airlock: useAirlock,
              gitConfigs: currentGitConfigs,
              preferV6: preferV6,
              env: finalEnv,
              RidTag: RidColored,
              cerSessionId: Cer_SessionId || null,
            ...
            const cloneArgs = forceH1
              ? ["clone", "--verbose", `--depth=${DFC.Depth}`, "--progress", actualCloneUrl, tempRepoPath]
              : ["clone", "--verbose", `--depth=${DFC.Depth}`, "--progress", "-b", targetBranch, actualCloneUrl, tempRepoPath];

            await MiaoPluginMBT.GitMutex.run(
              async () => {
            ...
            return MBTPipeControl(
              "git", cloneArgs, gitOptions, DFC.GitTimeout, null, null,
              (p) => {
                if (sitRep) sitRep(p);
              },
              null, MBTProcc
          ...
          const promiseChain = (async () => {
            try {
              activeTempPaths.add(tempRepoPath);
              const res = await executeGit();
              activeTempPaths.delete(tempRepoPath);
              return { ...res, ...meta };
            } catch (err) {
              activeTempPaths.delete(tempRepoPath);
              ...
              if (!isDowngrade) {
                const _strikeRes = PoseidonSpear.strike(node.name, coreReason);
                if (_strikeRes.punished) {
                  Hades.D(`${RidColored} | [Smart] 节点熔断: [${node.name}] ${_strikeRes.type} (冷却 ${(_strikeRes.coolingTime / 60000).toFixed(0)}m)`);
                } else {
                  Hades.D(`${RidColored} | [Smart] 节点失败但未熔断: [${node.name}] ${coreReason}`);
                }
              }
              ...
              setImmediate(() => {
                ...
                MiaoPluginMBT.CleanMutex.run(
                  async () => {
                    for (let i = 0; i < 2; i++) {
                      await Ananke.obliterate(tempRepoPath, 5, 800);
                      if (!(await Ananke.Audit(tempRepoPath, true))) break;
                      await common.sleep(1000 + i * 1000);
                    }
                  },
                  { id: `Cleanup:${uniqueSuffix}`, wait: 0, ttl: 8000 }
                ).catch(() => {});
              });
      ...
      nodePool.sort((a, b) => {
        const busy = MiaoPluginMBT._busyMap;
        const sa = PoseidonSpear.getNodeScore(a.name) * (busy.has(a.name) ? 0.4 : 1);
        const sb = PoseidonSpear.getNodeScore(b.name) * (busy.has(b.name) ? 0.4 : 1);
        if (Math.abs(sa - sb) > 0.01) return sb - sa;
        return getNodeLatency(a) - getNodeLatency(b);
      });
      let waveCount = 0;
      let lastWaveAt = Date.now();
      const WAVE_COOLDOWN = 60000;
      const firstWaveNodes = new Set();
      ...
      while (true) {
        ...
        if (Cer_SessionId) {
          const _allHs = typeof activeCRS?.isAllHandshaking === "function" && activeCRS.isAllHandshaking();
          const _maxProg = activeCRS?._leaderStatus?.maxProgress ?? 0;
          const _anyStage = activeCRS ? Array.from(activeCRS.tasks.values()).some((t) => {
          const s = t.context?.telemetry?.git_stage;
          const p = t.context?.telemetry?.progress ?? 0;
          return s === "compressing" || s === "resolving" || (s === "receiving" && p > 15);
          }) : false;
          const _hsMaxIdle = _allHs ? 180000 : _maxProg > 90 ? 240000 : _maxProg > 75 ? 180000 : _maxProg > 50 ? 135000 : _maxProg > 25 ? 120000 : _anyStage ? 180000 : 90000;
          const guardErr = cerberus.guard(Cer_SessionId, { maxPulseIdle: 90000, maxByteIdle: _hsMaxIdle, grace: _hsMaxIdle });
        ...
        const pickCount = Math.min(2, maxConcurrency);
        ...
        if (waveNodes.length === 0) {
          if (waveCount === 1 || !activeCRS) {
            waveNodes.push(githubNode);
            if (githubNode.name) firstWaveNodes.add(githubNode.name);
          } else {
            if (waveCount > 5) break;
            const _sinceLastWave = Date.now() - lastWaveAt;
            if (firstWaveNodes.size > 0 && _sinceLastWave < WAVE_COOLDOWN) {
              await common.sleep(2000); continue;
            }
            waveNodes.push(githubNode);
            if (githubNode.name) firstWaveNodes.add(githubNode.name);
          }
        ...
        for (const n of waveNodes) {
          if (!booked.includes(n.name)) {
            MiaoPluginMBT.grabNode(n.name);
            booked.push(n.name);
          }
        }
        ...
        const waveSlice = UL ? orderedWave.slice(0, UL.maxConcurrent) : orderedWave;
        let cumulativeDelay = 0;
        waveSlice.forEach((node, index) => {
          const delay = getStartDelay(node, index, riskMode, cumulativeDelay);
          cumulativeDelay = delay;
          activeCRS.addTask(node.name, createTaskFactory(node, forceH1), false, delay);
        });
        ...
        retryTimer = setInterval(() => {
          ...
          if (status.activeCount < maxConcurrent && nodePool.length > 0) {
            ...
            if (needBoost) {
              ...
              for (let i = 0; i < need && nodePool.length > 0; i++) {
                ...
                if (nextNode) {
                  activeCRS.addTask(nextNode.name, createTaskFactory(nextNode, forceH1), false, 0);
                  if (!booked.includes(nextNode.name)) {
                    MiaoPluginMBT.grabNode(nextNode.name);
                    booked.push(nextNode.name);
                  }
                  if (Cer_SessionId) {
                    try {
                      cerberus.pulse(Cer_SessionId, { event: "boost", state: "running" });
                    } catch {}
                  }
                }
              }
          ...
          if (!UL && status.activeCount === 0 && elapsed > 20000 && !activeCRS.tasks.has("GitHub") && useGitHubAsBackup) {
            activeCRS.addTask("GitHub", createTaskFactory(githubNode, forceH1), true, 1000);
          }
        }, 2000);
        ...
        try {
          ...
          await MiaoPluginMBT.GitMutex.run(
            async () => {
              if (repoNum === 1 && !(await Ananke.Audit(path.join(winnerResult.tempRepoPath, "CowCoo/resources/html")))) {
          ...
          PoseidonSpear.recordResult(winnerResult.nodeName, true, winnerResult.metrics?.ewma_speed || winnerResult.metrics?.instant_speed || 0);
          ...
          const lBytes = activeCRS.getStatus().leaderBytes || 0;
          activeCRS.stop();
          for (const n of waveNodes) {
          ...
          if (MODE === "NATIVE" && !AirlockRetry) {
            ...
            const airlockNodes = mirrorNodes.filter((n) => PoseidonSpear.isLive(n.name)).sort((a, b) => getNodeLatency(a) - getNodeLatency(b));
  ...
  static async UpstreamSyncRepo(e, RepoNum, localPath, RepoName, RepoUrl, branch, isScheduled, logger, senseChain) {
    return await MiaoPluginMBT.GitMutex.run(
      async () => {
        ...
        const largeRepoNums = cfg.LargeRepoNums ?? DFC.LargeRepoNums ?? [];
        const isLargeRepo = largeRepoNums.includes(RepoNum);
        const basePullTimeout = Math.max(
          Number(cfg.PullTimeout ?? DFC.PullTimeout) || 0,
          RepoNum === 1 || isLargeRepo ? Number(cfg.PullTimeoutCore ?? DFC.PullTimeoutCore) || 480000 : Number(cfg.PullTimeoutAss ?? DFC.PullTimeoutAss) || 240000
        );
        ...
        const initial_pipe_opts = {
          inheritEnv: execCtx.inheritEnv,
          env: execCtx.gitEnv,
          gitConfigs: smartConfigs,
          airlock: !!execCtx.isAirlock,
          preferV6: mode === Proteus.State.V6_TURBO,
          constraints: { zombieThreshold: 60 * 1000, low_Speed_Limit: 10240, low_Speed_Strikes: 3 }
        };

        const getModePullTimeout = (m) => {
          const nativeCap = RepoNum === 1 || isLargeRepo ? 240000 : 180000;
          const agentCap = RepoNum === 1 || isLargeRepo ? 300000 : 210000;
          if (m === Proteus.State.NATIVE || m === Proteus.State.V6_TURBO) return Math.max(90000, Math.min(basePullTimeout, nativeCap));
          if (m === Proteus.State.USER_AGENT || m === Proteus.State.IDLE_AGENT) return Math.max(120000, Math.min(basePullTimeout, agentCap));
              ...
              if ((oldCommit && newCommit && oldCommit !== newCommit) || result.wasHardReset) {
                ...
                if (oldCommit && newCommit) {
                  try {
                    ...
                    result.diffStat = {
                      insertions: parseInt((diffOut.match(/(\d+)\s+insertion/) || [0, 0])[1]),
          ...
          if (isNetErr) {
            ...
            try {
              ...
              if (survivors.length > 0) {
                ...
                orderNodes = (DFC.F2Pool || []).filter((n) => n.name !== "GitHub" && n.ClonePrefix).sort((a, b) => (a.priority || 999) - (b.priority || 999));
              }
            } catch (e) {
            ...
            for (const winner of orderNodes) {
              ...
              try {
                ...
                const lsResult = await MBTPipeControl("git", ["ls-remote", "--heads", "origin", branch], { cwd: localPath, ...cleanOpts }, 8000).catch(() => ({ stdout: "" }));
            ...
            if (!syncResult.success && github_url) {
              const stage_rows = [
                ...
                {
                  name: "Stage-2(H1+剥离)",
                  opts: { inheritEnv: false, env: null, gitConfigs: ["http.version=HTTP/1.1", "http.proxy=", "https.proxy=", "core.gitProxy="] }
                }
              ];
              ...
              for (const stage of stage_rows) {
                try {
                  ...
                  await rollback_origin(`${stage.name}-error`);
                }
              }
            }
          } else {
        ...
        try {
          const rawLog = (
            await MBTPipeControl("git", ["log", `-n ${RepoNum === 1 ? 5 : 3}`, `--date=${DFC.logDateFormat}`, `--pretty=format:%cd [%h]%n%s%n%b`], { cwd: localPath }, 5000)
          ).stdout;
          if (rawLog) {
            const entries = rawLog.split(/(?=\d{2}-\d{2}\s\d{2}:\d{2}\s+\[)/).filter((s) => s.trim());
            const BtnFaceUrl = toFileUrl(path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "img", "icon", "null-btn.png"));
  ...
  static async TestGitVoice(ClonePrefix, nodeName, logger = getCore()) {
    ...
    const TIMEOUT = 15000;
    const Hades = getHades(logger);
    const repoPool = (await Nomos.CleanDataPool("RepoPool", logger)).map((url) => url.match(/github\.com\/([^/]+\/[^/]+?)(?:\.git)?$/)?.[1]).filter(Boolean);
    ...
    while (attempt < Max_Retries) {
      ...
      const cleanPrefix = ClonePrefix ? ClonePrefix.replace(/\/$/, "") : "";
      const actualUrl = isDirect ? `https://github.com/${targetRepo}.git` : `${cleanPrefix}/github.com/${targetRepo}.git`;
      const startTime = Date.now();
  ...
  static async ProvisionPhase(e, logger = getCore(), stage = "full") {
    ...
    try {
      ...
      MiaoPluginMBT.MBTConfig = await Ananke.loadingConfig(MiaoPluginMBT.Paths.ConfigFilePath, DFC, Hades);
  ...
  static async AdaptiveSpeed(HttpResultMap, GitRTTs, logger = getCore(), preferDirect = false) {
    ...
    const gitEliteNodesMap = new Map(GitRTTs.filter((n) => n.gitResult?.success).map((n) => [n.name, n.gitResult]));
  ...
  static async SendMasterMsg(msg, e = null, delay = 0) {
    if (MasterMsg?.sendMasterMsg) return MasterMsg.sendMasterMsg(msg, e, delay);
  }
  ...
  async ReconcileTask() {
    ...
    try {
      Nomos.ensureCRP({ force: true, logger: this.logger, busyWaitMs: 60000 }).catch(() => {});
      HasAnyChanges = await this.Reconcile(null, true);
      this.logger.debug(`定时更新完成,有更新:${HasAnyChanges}`);
    } catch (error) {
      this.logger.error(`定时更新异常:`, error);
      const Report = DocHub._diagnose("定时更新", error, "");
      this.logger.error(`--- 定时更新诊断 ----\n${Report.summary}\n${Report.suggestions}\n---`);
    } finally {
      const duration = ((Date.now() - startTime) / 1000).toFixed(1);
    ...
    const cleanTasks = [
      ...
      (async () => {
        try {
          ...
          if (e.code !== "ENOENT") this.logger.warn(`HTML清理异常:${e.message}`);
        }
      })(),
      ...
      (async () => {
        try {
          ...
          if (fs.existsSync(gitDir)) {
            await MBTPipeControl("git", ["gc", "--auto", "--quiet"], { cwd: path.dirname(gitDir), Rid: "CronGC", caDisabled: true }, 15000).catch(() => {});
          }
        } catch {}
      })()
    ];
  ...
  async Provision(e) {
    ...
    if (!(await MiaoPluginMBT.OpsGate(e, "Provision", { cooldown: 120, deferSet: true }))) return true;

    try {
      const RPD = await Nomos.ensureCRP({
        force: true,
        logger: Hades,
        busyWaitMs: 60000
      });
      if (!RPD) {
        throw new Error("前置库同步失败");
      }
      ...
      MiaoPluginMBT.OpsSetCooldown("Provision", e.user_id, 120);
      ...
      const deployRepo = async (repoTask) => {
        ...
        const result = await MiaoPluginMBT.SmartTaskHeavy(runtimeContext, repoNum, repoUrl, branch, repoPath, e, Hades, TheGrid, MBTProcc, null, null, sessionId);
        ...
        if (!result.success) {
          ...
          else Hades.W(`附属仓库 ${repoNum} 下载失败: ${result.error?.message}`);
          return result;
        }
        ...
        if (repoNum === 1) {
          ...
          try {
            const coreTplResult = await Nomos.getTemplate("core_repo_download.html");
      ...
      if (!allSuccess) MiaoPluginMBT.OpsClearCooldown("Provision", e.user_id);
    }
    return true;
  }
  ...
  async _Debrief(e, repoManifest, startTime, allSuccess) {
    ...
    const results = repoManifest
      .filter((r) => !HiddenRepos.includes(r.repo))
      .map((r) => {
        ...
        if (r.status === "fail" && r.error) {
          const formattedError = DocHub._diagnose(`下载${name}`, r.error, "");
          let errorReportText = `--- ${name} 下载失败 ---\n`;
          errorReportText += `${formattedError.summary}\n\n`;
          errorReportText += `**可能原因与建议**\n${formattedError.suggestions}\n\n`;
          if (r.error.stderr || r.error.stdout) {
    ...
    try {
      const { success: tplOk, data: tplData } = await Nomos.getTemplate("download.html");
      let imgBuffer = null;
      ...
      if (allSuccess) {
        ...
        await Pheme.send(e, `咕咕牛部分仓库下载失败，请检查日志或重试。${failDetailText}`);
        if (errorList.length > 0 && e) {
          await common.sleep(500);
          try {
            await Pheme.forward(e, errorList, "咕咕牛下载失败详情");
          } catch (fwdError) {
            this.logger.error(`发送详细错误合并消息失败:`, fwdError);
  ...
  async TestVoice(e) {
    ...
    const taskCoreProbe = MiaoPluginMBT.TestCaVoice(Hades).then(async (httpResults) => {
      ...
      const gitPromises = validNodes.map((n) => MiaoPluginMBT.TestGitVoice(n.ClonePrefix, n.name, Hades).then((res) => ({ name: n.name, gitResult: res })));
    ...
    try {
      ...
      await DocHub.report(e, "咕咕牛测速", err, "", Hades);
    }
    return true;
  }
    ...
    try {
      ...
      const extractCC = (src) => {
        return (
          ...
          src?.country_code ||
          src?.country ||
          src?.geoData?.countryCode ||
          src?.geoData?.country_code ||
          src?.geoData?.country ||
          src?.data?.country_code ||
          src?.data?.country ||
          ""
        );
      };
      ...
      const tplResult = await Nomos.getTemplate("speedtest.html");
      if (!tplResult.success) throw new Error("模板加载失败");

      const p1Stats = httpResults
      ...
      const shotName = Nomos.isDeployed ? "US-SpeedTest" : "DL-SpeedTest";
      ...
      await Pheme.genFail(e, "speedtest");
      await DocHub.report(e, "测速报告生成", err, "", Hades);
    }
  ...
  async Reconcile(e, isScheduled = false) {
    ...
    try {
      ...
      if (hasMissingRepo && !isScheduled && e) {
        await Pheme.quote(e, "咕咕牛部分附属仓库未下载，建议先`#下载咕咕牛`补全。");
      }

      if (!isScheduled && e) await Pheme.quote(e, "咕咕牛开始检查更新...");
      ...
      if (ConfigChanged) {
        await Ananke.SaveConfig(MiaoPluginMBT.Paths.ConfigFilePath, MiaoPluginMBT.MBTConfig, Hades);
      }
      ...
      if (UpdateRenderFlag) {
        imgBuffer = await Morpheus.shot("Update", {
          tplFile: path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "html", "sync", "update.html"),
          data: ViewProps,
          logger: Hades,
          pageBoundingRect: { selector: ".capture-frame" },
          MorpheusSignal: true
      ...
      if (JavaScriptSyncStatus) {
        const ResMsg = `检测到插件核心逻辑已更新！为确保所有功能正常建议重启机器人。`;
        if (!isScheduled && e) {
      ...
      Hades.E(`更新流程顶层异常:`, error);
      if (!isScheduled && e) await DocHub.report(e, "更新咕咕牛", error, `耗时: ${duration}s`, Hades);
      else {
        const Report = DocHub._diagnose("更新咕咕牛(定时)", error, "");
        Hades.E(`--- 定时更新顶层失败 ----\n${Report.summary}\n${Report.suggestions}\n---`);
      }
      return false;
    }
  ...
  async ManRepo(e) {
    ...
    const failures = results.filter((r) => !r.success).map((r) => `${r.type} ${r.path} (${r.error?.message || "Failed"})`);
  ...
  async CheckStatus(e) {
    ...
    try {
      ...
      const worker = await HotModule.load("worker", { dirNames: Nomos.DirNames });
      const repoTasks = ScanRepos.map((r) =>
        Promise.all([Tianshu.ResolveGitNode(r.path), Ananke.measure(path.join(r.path, ".git")), Nomos.getRepoStatus(r.path).then((res) => res.sha.slice(0, 15))])
      );
      ...
      const Universe = Nomos.Universe;
      const KeyDir = {};
      for (const [k, v] of Object.entries(Universe)) {
        KeyDir[v.dirName] = k;
      }
      ...
      ScanRepos.forEach((repo, index) => {
        ...
        statsData.repos[repo.name] = {
          ...
          gitSize: gitSize,
          filesSizeFormatted: "计算中...",
          gitSizeFormatted: "计算中...",
          sizeFormatted: "计算中..."
        };
      ...
      for (const cnName of Object.keys(statsData.scan.gameSizes)) {
        fmtTasks.push(Ananke.measure(statsData.scan.gameSizes[cnName], true).then((s) => (statsData.scan.gameSizesFormatted[cnName] = s)));
      }
      for (const repoName of Object.keys(statsData.repos)) {
        const r = statsData.repos[repoName];
        fmtTasks.push(Ananke.measure(r.filesSize, true).then((s) => (r.filesSizeFormatted = s)));
        fmtTasks.push(Ananke.measure(r.gitSize, true).then((s) => (r.gitSizeFormatted = s)));
        fmtTasks.push(Ananke.measure(r.filesSize + r.gitSize, true).then((s) => (r.sizeFormatted = s)));
      }
      statsData.scan.gitSize = GitWorkingSize;
      statsData.scan.size = statsData.scan.filesSize + GitWorkingSize;
      fmtTasks.push(Ananke.measure(GitWorkingSize, true).then((s) => (statsData.scan.gitSizeFormatted = s)));
      fmtTasks.push(Ananke.measure(statsData.scan.size, true).then((s) => (statsData.scan.sizeFormatted = s)));
      fmtTasks.push(Ananke.measure(statsData.scan.filesSize, true).then((s) => (statsData.scan.filesSizeFormatted = s)));
      let RobotSize = robotSizeRaw;
      let robotSizeFormatted = "0 B";
      fmtTasks.push(Ananke.measure(RobotSize, true).then((s) => (robotSizeFormatted = s)));
      await Promise.all(fmtTasks);
      ...
      const imgBuffer = await Morpheus.shot("Status", {
        tplFile: path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "html", "tools", "status.html"),
        data: ViewProps,
        logger: Hades,
        pageBoundingRect: { selector: ".container" },
        MorpheusSignal: true,
  ...
  async _TriggerMapGen(e, logger) {
    ...
    for (const task of tasks) {
      ...
      const imgBuffer = await MBTMapTileAss.render(task.key, task.width, Hades);
  ...
  async MuB(e) {
    ...
    if (addMatch || delMatch) {
      ...
      try {
        ...
        Hades.E(`${actionVerb}未知异常:`, err);
        await DocHub.report(e, `${actionVerb}图片`, err, `输入: ${rawInput}`);
        await Pheme.quote(e, `操作失败: ${err.message}`);
      }
      return true;
    }
  ...
  async QueryData(e) {
    ...
    try {
      ...
      const logger = getHades(this.logger);
      logger.E(`查看图库异常:`, error);
      await DocHub.report(e, "查看图库", error, "", logger);
      return true;
    }
  }
  ...
  async VisSplashes(e) {
    ...
    try {
      ...
      if (!roleFolderPath) {
        Hades.W(`未在目标插件目录找到角色 '${primaryName}' 文件夹。`);
      ...
      let allimgFiles = [];
      try {
        const entries = await Ananke.readDir(roleFolderPath);
      ...
      for (let pageNum = 1; pageNum <= pageCount; pageNum++) {
        ...
        const imgBuffer = await Morpheus.shot(`Vis-${primaryName}-P${pageNum}`, {
          tplFile: path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "html", "filter", "visualize.html"),
          data: ViewProps,
          logger: logger,
  ...
  async ExPhFile(e) {
    if (ExportMod?.exportFile) return ExportMod.exportFile(e, (ev) => this.CheckInit(ev), this.logger);
    if (!(await this.CheckInit(e))) return true;
    return Pheme.notReady(e);
  }

  async Help(e) {
    const isInstalled = Nomos.isDeployed;
    const logger = getHades(this.logger);

    let HelpTpl = "";
    const tplResult = await Nomos.getTemplate("help.html");
    ...
    if (HelpTpl) {
      try {
        ...
        await DocHub.report(e, "帮助面板", renderErr, "", logger);
        HelpTpl = "";
      }
    }
  ...
  async MBTOpsDeck(e, StatusMsg = "") {
    ...
    try {
      const ViewProps = {
        ...
        preemptStatus: mapToggle("Preempt", DFC.Preempt),
        MihoyoSplashStatus: mapToggle("MihoyoSplash", DFC.MihoyoSplash),
        sr18Status: mapToggle("SR18", false),
        RenderScale: { value: config.RenderScale ?? DFC.RenderScale }
      };

      const imgBuffer = await Morpheus.shot("Settings", {
        tplFile: path.join(MiaoPluginMBT.Paths.OpsPath, "resources", "html", "tools", "setpanel.html"),
        data: ViewProps,
        logger: logger,
        pageBoundingRect: { selector: ".container" },
        padding: 25,
      ...
      if (imgBuffer) {
        ...
        const fallbackMsg = StatusMsg ? `${StatusMsg}\n(面板渲染失败，请检查日志)` : `设置面板生成失败，请查看后台日志。`;
  ...
  async RouteOpsHub(e) {
    ...
    const subMatch = msg.match(/^#咕咕牛设置(ai|彩蛋|横屏|净化等级|渲染精度|SR18|原图拦截|官方立绘)(开启|关闭|[0-9]+)$/i);
    if (subMatch) {
  ...
  async MutateSubState(e, key, valRaw) {
    ...
    const refreshList = async (logText, errText, syncRepo = false) => {
      ...
      try {
        ...
        if (errText) Hades.E(`${errText}:`, err);
      }
    };
    const strategies = {
      总开关: {
    ...
    if (mutation.changed && strategy.sideEffect) {
      try {
        ...
        Hades.E(`${strategy.name}副作用执行失败:`, err);
      }
    }
    ...
    try {
      ...
      Hades.E("面板渲染失败", err);
      await DocHub.report(e, `设置${strategy.name}面板`, err, mutation.msg, logger);
    }
    return true;
  }
}
...
class SleeperAgent extends plugin {
  ...
  async PreemptPh(e) {
    ...
    if (replyId) {
      return SleeperAgent._interrogate(e, replyId);
    }
  ...
  static async _interrogate(e, sourceMsgId) {
    const _cfg = MiaoPluginMBT.MBTConfig || {};
    if (!(_cfg.Preempt ?? DFC.Preempt)) {
      return true;
    }
    const ProbeKeys = [
      ...
      try {
        ...
        if (dataJson) {
          ...
          if (imgPath.startsWith("http")) {
            ...
            absolutePath = path.join(YzPath, "plugins", "miao-plugin", "resources", imgPath);
          } else if (type === "zzz") {
            absolutePath = path.join(MiaoPluginMBT.Paths.Target.ZZZCRE, imgPath);
          } else if (type === "waves") {
            absolutePath = path.join(MiaoPluginMBT.Paths.Target.WavesCRE, imgPath);
          } else {
            continue;
          }
...
const CowCoo_Rules = [
  ...
  [/^#(?:(启用|禁用)咕咕牛|咕咕牛设置(ai|彩蛋|横屏|净化等级|渲染精度|SR18|原图拦截|官方立绘)(开启|关闭|[0-9]+))$/i, "RouteOpsHub", "master"],
  [/^#可视化\s*.+$/i, "VisSplashes"]
...
const apps = { MiaoPluginMBT, SleeperAgent, ...(CommunityMBT ? { CommunityMBT } : {}), ...(FetcherMBT ? { FetcherMBT } : {}), ...(Sentinel ? { Sentinel } : {}) };
export { apps, MiaoPluginMBT, SleeperAgent, MBTPagination, CommunityRules, FetcherRules };