...
export class MiaoPluginMBT extends plugin {
    ...
    try {
      ...
      if (!Repo1Exists) {
        ...
        try {
          ...
          logger.info(
            `${logPrefix} [核心优先下载] 核心仓库下载成功 (${coreRepoResult.nodeName})。`
          );
        } catch (err) {
          logger.error(
            `${logPrefix} [核心优先下载] 核心仓库下载过程中发生意外错误:`,
            err
          );
          coreRepoResult = {
            repo: 1,
      ...
      if (Repo2UrlConfigured && !Repo2Exists) {
        logger.info(
          `${logPrefix} [核心优先下载] 添加附属仓库 (二号) 下载任务。`
        );
        subsidiaryPromises.push(
          MiaoPluginMBT.DownloadRepoWithFallback(
            ...
            .catch((err) => {
              logger.error(
                `${logPrefix} [核心优先下载] 附属仓库 (二号) 下载 Promise 捕获到错误:`,
                err
              );
              return {
                repo: 2,
      ...
      if (Repo3UrlConfigured && !Repo3Exists) {
        logger.info(
          `${logPrefix} [核心优先下载] 添加附属仓库 (三号) 下载任务。`
        );
        subsidiaryPromises.push(
          MiaoPluginMBT.DownloadRepoWithFallback(
            3, // repoNum = 3
            ...
            .catch((err) => {
              logger.error(
                `${logPrefix} [核心优先下载] 附属仓库 (三号) 下载 Promise 捕获到错误:`,
                err
              );
              return {
                repo: 3,
        ...
        logger.info(`${logPrefix} [核心优先下载] 附属仓库 (三号) 已存在。`);
        subsidiaryResults.push({
          repo: 3,
          success: true,
          nodeName: "本地",
        ...
        logger.info(`${logPrefix} [核心优先下载] 附属仓库 (三号) 未配置。`);
      }

      if (subsidiaryPromises.length > 0) {
        logger.info(
          `${logPrefix} [核心优先下载] 等待 ${subsidiaryPromises.length} 个附属仓库下载完成...`
        );
        const settledResults = await Promise.allSettled(subsidiaryPromises);
        logger.info(
          `${logPrefix} [核心优先下载] 所有附属仓库 Promise 已完成 (settled)。`
        );
        settledResults.forEach((result) => {
          if (result.status === "fulfilled") {
            ...
            if (result.value.success) {
              logger.info(
                `${logPrefix} [核心优先下载] 附属仓库 (${result.value.repo}号) 下载成功 (${result.value.nodeName})。`
              );
            } else {
              logger.error(
                `${logPrefix} [核心优先下载] 附属仓库 (${result.value.repo}号) 下载失败 (${result.value.nodeName})。`
              );
              logger.error(
                `${logPrefix} [核心优先下载] 失败详情:`,
                result.value.error
              );
            }
          } else {
            logger.error(
              `${logPrefix} [核心优先下载] 一个附属仓库 Promise rejected:`,
              result.reason
            );
          }
        });
      ...
      subsidiaryResults.forEach((res) => {
        ...
        subsidiaryStatusLines.push(
          `       - ${repoDisplayName}：${statusText}`
        );
      });
      ...
      // --- 执行下载后设置 ---
      logger.info(
        `${logPrefix} [核心优先下载] 开始执行 RunPostDownloadSetup...`
      );
      await MiaoPluginMBT.RunPostDownloadSetup(e, logger);
      logger.info(
        `${logPrefix} [核心优先下载] RunPostDownloadSetup 执行完成。`
      );
      ...
      for (const res of subsidiaryResults) {
        if (
          ...
          if (res.repo === 2) {
            ...
            repoDisplayName = "二号仓库";
          }
          if (res.repo === 3) {
            repoPath = MiaoPluginMBT.paths.LocalTuKuPath3;
            repoDisplayName = "三号仓库";
          }
          if (repoPath) {
            const gitLogSub = await MiaoPluginMBT.GetTuKuLog(
              1,
              repoPath,
              logger
            );
      ...
      logger.info(
        `${logPrefix} [核心优先下载] 流程结束，耗时 ${duration} 秒。`
      );
    }
    return true;
  }
  ...
  async UpdateTuKu(e, isScheduled = false) {
    ...
    const startTime = Date.now();
    if (!isScheduled && e)
      await e.reply("『咕咕牛🐂』开始检查更新，稍等片刻...", true);
    this.logger.info(
      `${this.logPrefix} [更新流程] 开始 @ ${new Date(startTime).toISOString()}`
    );
    ...
    try {
      ...
      if (Repo1Exists) {
        ...
        if (!Repo1Success && !isScheduled)
          finalUserMessage = "核心仓库(一号)更新失败了...";
        else if (Repo1Success) {
          gitLogRepo1 = result1.log || "";
          if (Repo1Updated)
            this.logger.info(`${this.logPrefix} 核心仓库(一号)更新了内容。`);
          else
            this.logger.info(
              `${this.logPrefix} 核心仓库(一号)已经是最新版本。`
            );
        }
      } else {
        Repo1Success = false; // 核心不存在，更新失败
      }

      // 更新附属仓库 (只有在核心仓库更新成功后才进行)
      if (Repo1Success) {
        // 更新二号仓库
        if (Repo2UrlConfigured && Repo2Exists) {
          ...
          if (!Repo2Success && !isScheduled && !finalUserMessage)
            finalUserMessage = "附属仓库(二号)更新失败了...";
          else if (Repo2Success) {
            gitLogRepo2 = result2.log || "";
            if (Repo2Updated)
              this.logger.info(`${this.logPrefix} 附属仓库(二号)更新了内容。`);
            else
              this.logger.info(
                `${this.logPrefix} 附属仓库(二号)已经是最新版本。`
              );
          }
        } else if (Repo2UrlConfigured && !Repo2Exists) {
          this.logger.info(
            `${this.logPrefix} 附属仓库(二号)未下载，跳过更新。`
          );
        } else if (!Repo2UrlConfigured) {
          Repo2Success = true; // 未配置视为成功
        }

        // 更新三号仓库
        if (Repo3UrlConfigured && Repo3Exists) {
          const result3 = await MiaoPluginMBT.UpdateSingleRepo(
            null,
            3,
            MiaoPluginMBT.paths.LocalTuKuPath3,
            "三号仓库",
          ...
          Repo3Success = result3.success;
          Repo3Updated = result3.hasChanges;
          if (!Repo3Success && !isScheduled && !finalUserMessage)
            finalUserMessage = "附属仓库(三号)更新失败了...";
          else if (Repo3Success) {
            gitLogRepo3 = result3.log || "";
            if (Repo3Updated)
              this.logger.info(`${this.logPrefix} 附属仓库(三号)更新了内容。`);
            else
              this.logger.info(
                `${this.logPrefix} 附属仓库(三号)已经是最新版本。`
              );
          }
        } else if (Repo3UrlConfigured && !Repo3Exists) {
          this.logger.info(
            `${this.logPrefix} 附属仓库(三号)未下载，跳过更新。`
          );
        } else if (!Repo3UrlConfigured) {
          Repo3Success = true; // 未配置视为成功
        }
      }

      overallHasChanges = Repo1Updated || Repo2Updated || Repo3Updated;
      const overallSuccess = Repo1Success && Repo2Success && Repo3Success;

      if (overallSuccess) {
        if (overallHasChanges) {
          if (!isScheduled && e)
            await e.reply(`${this.logPrefix} 检测到更新，正在应用变更...`);
          ...
          if (!isScheduled && e) {
            ...
            if (gitLogRepo2 && Repo2Updated)
              logMessages.push(
                `--- 附属仓库(二号)更新记录 ---\n${gitLogRepo2}`
              );
            if (gitLogRepo3 && Repo3Updated)
              logMessages.push(
                `--- 附属仓库(三号)更新记录 ---\n${gitLogRepo3}`
              );
            ...
            this.logger.info(
              `${this.logPrefix} [定时] 检测到更新，准备通知主人...`
            );
            const latestLogCombined =
              gitLogRepo1 ||
              (await MiaoPluginMBT.GetTuKuLog(
                1,
                MiaoPluginMBT.paths.LocalTuKuPath,
                this.logger
              ));
        ...
        if (!isScheduled && e) {
          if (!finalUserMessage)
            finalUserMessage = "『咕咕牛』更新过程中出错了，快去看看日志！";
          await e.reply(finalUserMessage);
        }
