...
const TRIGGERABLE_ITEMS = Object.freeze([
  ...
  {
    id: 21,
    name: "报告: 下载完成-核心成功,附属部分失败",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“下载完成报告”：核心库成功，一个附属库失败，一个已存在。",
    type: "SIMULATE_REPORT_DL_FINAL_MIXED_RESULT",
  },
  { id: 22, name: "报告: 下载完成-全部失败", category: "核心图片报告模拟", description: "生成并发送一张模拟的“下载完成报告”：所有尝试的仓库均下载失败。", type: "SIMULATE_REPORT_DL_FINAL_ALL_FAIL" },
  {
    ...
    name: "报告: 下载完成-全部成功",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“下载完成报告”：所有配置的仓库均下载成功。",
    type: "SIMULATE_REPORT_DL_FINAL_ALL_SUCCESS",
  },
  ...
  // **更新完成报告相关 (ID 30-35)**
  {
    ...
    name: "报告: 更新完成-无变化",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“更新完成报告”：所有仓库均已是最新，无任何文件变更。",
    type: "SIMULATE_REPORT_UP_FINAL_NO_CHANGES",
  },
  {
    id: 31,
    name: "报告: 更新完成-核心有变(常规),附属无变",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“更新完成报告”：核心库有新的常规更新，附属库无变化。",
    type: "SIMULATE_REPORT_UP_FINAL_CORE_CHANGED_NORMAL",
  },
  {
    id: 32,
    name: "报告: 更新完成-核心强制同步,附属OK",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“更新完成报告”：核心库因冲突被强制同步 (reset --hard)，附属库正常更新或无变化。",
    type: "SIMULATE_REPORT_UP_FINAL_CORE_FORCED_SYNC",
  },
  {
    id: 33,
    name: "报告: 更新完成-核心失败,附属成功",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“更新完成报告”：核心库更新失败，附属库更新成功。",
    type: "SIMULATE_REPORT_UP_FINAL_CORE_FAIL_ASSIST_OK",
  },
  { id: 34, name: "报告: 更新完成-全部失败", category: "核心图片报告模拟", description: "生成并发送一张模拟的“更新完成报告”：所有仓库均更新失败。", type: "SIMULATE_REPORT_UP_FINAL_ALL_FAIL" },
  {
    id: 35,
    name: "报告: 更新完成-全部有变(常规)",
    category: "核心图片报告模拟",
    description: "生成并发送一张模拟的“更新完成报告”：所有配置的仓库均有新的常规更新。",
    type: "SIMULATE_REPORT_UP_FINAL_ALL_CHANGED_NORMAL",
  },
  ...
  // --- 其他业务逻辑状态 (ID 50+) ---
  {
    ...
    name: "逻辑: 截图过程返回空值",
    category: "业务逻辑状态",
    description: "模拟任何截图操作后，Puppeteer 未抛错但返回了 null/undefined (可能是空白图)。预期：插件记录错误，可能回复用户生成失败。",
    type: "SIMULATE_LOGIC_RENDER_BLANK_IMAGE",
  },
  {
    id: 51,
    name: "逻辑: 配置文件恢复并通知",
    category: "业务逻辑状态",
    description: "模拟配置加载时触发恢复，成功恢复并(尝试)通知主人。预期：日志记录，主人收到私聊。",
    type: "SIMULATE_LOGIC_CONFIG_RECOVER_NOTIFY",
  },
]);
...
async function safeDelete(targetPath, maxAttempts = 3, delay = 1000) {
  if (targetPath === null || typeof targetPath === "undefined" || targetPath === "") {
    // logger.debug(`${Default_Config.logPrefix} [安全删除] 路径为空或无效，跳过删除。`); //调式日志-精简
    return true;
  }

  let attempts = 0;
  const logger = global.logger || console; // 确保 logger 定义
  while (attempts < maxAttempts) {
    try {
      await fsPromises.rm(targetPath, { recursive: true, force: true });
      // logger.debug(`${Default_Config.logPrefix} [安全删除] 成功删除 ${targetPath}`); //调式日志-精简
      return true;
    } catch (err) {
      if (err.code === ERROR_CODES.NotFound) {
        // logger.debug(`${Default_Config.logPrefix} [安全删除] 路径 ${targetPath} 不存在，视为已删除。`); //调式日志-精简
        return true;
      }
      if ([ERROR_CODES.Busy, ERROR_CODES.Perm, ERROR_CODES.NotEmpty].includes(err.code)) {
        ...
        if (attempts >= maxAttempts) {
          ...
          return false; // 改为返回 false 表示删除失败
        }
  ...
  return false; // 正常情况下不应该执行到这里，但作为兜底
}
...
export class MiaoPluginMBT extends plugin {
  ...
  /**
   * @description 定时任务执行入口，调用 UpdateTuKu 并处理日志和潜在的通知。
   */
  async RunUpdateTask() {
    ...
    try {
      ...
      // UpdateTuKu(null, true) 内部会尝试报告错误给主人
    } finally {
  ...
  async ManualRunUpdateTask(e) {
    ...
    try {
      // UpdateTuKu 内部会在 isScheduled=true 且有变更/错误时尝试通知主人
    ...
    // 只有在没有错误发生时才回复成功或无变化的消息（错误消息由 ReportError 处理）
    if (!taskError) {
    ...
    // 如果 taskError 不为 null，则 ReportError 已经向 e 回复了错误信息，此处不再回复。
    ...
    try {
      ...
      if (Repo2UrlConfigured && !Repo2Exists) {
        ...
        subsidiaryResults.push({
          ...
          nodeName: "本地",
          error: null,
          gitLog: await MiaoPluginMBT.GetTuKuLog(1, MiaoPluginMBT.paths.LocalTuKuPath2, logger),
        });
      } // 已存在也获取日志
      else {
      ...
      if (Repo3UrlConfigured && !Repo3Exists) {
        ...
        subsidiaryResults.push({
          ...
          nodeName: "本地",
          error: null,
          gitLog: await MiaoPluginMBT.GetTuKuLog(1, MiaoPluginMBT.paths.LocalTuKuPath3, logger),
        });
      } // 已存在也获取日志
      else {
      ...
      if (subsidiaryPromises.length > 0) {
        ...
        for (const result of settledResults) {
          if (result.status === "fulfilled") {
            ...
            if (resValue.success && resValue.nodeName !== "本地" && resValue.nodeName !== "未配置") {
  ...
  async UpdateTuKu(e, isScheduled = false) {
    ...
    try {
        ...
        if (!isScheduled && e) await e.reply(img);
          ...
          for (const masterId of Bot.master) {
            try {
              ...
              try {
                await Bot.pickUser(masterId).sendMsg(fallbackMsgMaster);
              } catch (innerSendErr) {
                logger.error(`${logPrefix} [定时更新] 发送文本回退给主人 ${masterId} 也失败:`, innerSendErr);
              }
            }
          }
      ...
      if (fs.existsSync(tempRenderDir))
        try {
          await safeDelete(tempRenderDir);
        } catch (delErr) {}
    }
  ...
  async ManageTuKu(e) {
    ...
    // 删除主要仓库和公共资源目录
    const pathsToDeleteDirectly = [MiaoPluginMBT.paths.LocalTuKuPath, MiaoPluginMBT.paths.LocalTuKuPath2, MiaoPluginMBT.paths.LocalTuKuPath3, MiaoPluginMBT.paths.commonResPath].filter(Boolean);
  ...
  async CheckStatus(e) {
    ...
    try {
      ...
      tempHtmlFilePath = path.join(MiaoPluginMBT.paths.tempHtmlPath, `status-${Date.now()}-${Math.random().toString(16).slice(2)}.html`);
      tempImgFilePath = path.join(MiaoPluginMBT.paths.tempImgPath, `status-${Date.now()}-${Math.random().toString(16).slice(2)}.png`);
  ...
  async ManageUserBans(e) {
    ...
    if ((msg.startsWith("#咕咕牛封禁 ") || msg.startsWith("#咕咕牛解禁 ")) && !isMaster) return e.reply(`${logPrefix} 只有主人才能进行封禁或解禁操作哦~`);
        ...
        for (let batchNum = 1; batchNum <= totalBatchesPurified; batchNum++) {
          ...
          try {
            ...
            if (imgBatch) {
              ...
              logger.info(`${logPrefix} [封禁列表] 自动屏蔽列表第 ${batchNum}/${totalBatchesPurified} 批图片生成成功。`);
            } else {
              logger.error(`${logPrefix} [封禁列表] 生成自动屏蔽列表第 ${batchNum}/${totalBatchesPurified} 批截图失败。`);
              forwardListPurified.push(`[❌ 第 ${batchNum}/${totalBatchesPurified} 批渲染失败]`);
            }
        ...
        if (forwardListPurified.length > 1) {
          try {
            ...
            if (forwardMsgPurified) {
              ...
              logger.error(`${logPrefix} [封禁列表] 创建自动屏蔽列表合并消息失败 (makeForwardMsg 返回空)。`);
              await e.reply("生成合并的自动屏蔽列表消息失败 (内部错误)。", true);
            }
    ...
    if (addMatch || delMatch) {
      ...
      if (!targetIdentifierRaw) {
        return e.reply(`要${actionVerb}哪个图片呀？格式：#咕咕牛${actionVerb}角色名+编号`, true);
      }
  ...
  async FindRoleSplashes(e) {
    ...
    try {
      ...
      this.logger.info(`${this.logPrefix} [查看] 角色 ${standardMainName} 共 ${totalItems} 张图片，将分 ${totalBatches} 批发送。`);
      ...
      for (let batchNum = 1; batchNum <= totalBatches; batchNum++) {
        ...
        for (let i = 0; i < currentBatchData.length; i++) {
          ...
          const isPurified = MiaoPluginMBT.CheckIfPurifiedByLevel(item, MiaoPluginMBT.MBTConfig.PFL ?? Default_Config.defaultPfl);
        ...
        if (currentForwardList.length > 1) {
          try {
            const forwardMsg = await common.makeForwardMsg(e, currentForwardList, `[${standardMainName}] 图库详情 (${batchNum}/${totalBatches})`);
            if (forwardMsg) {
              await e.reply(forwardMsg);
            ...
            this.logger.error(`${this.logPrefix} [查看] 发送第 ${batchNum}/${totalBatches} 批合并转发消息失败:`, sendError);
  ...
  async VisualizeRoleSplashes(e) {
    ...
    try {
      ...
      if (!roleFolderPath) {
        ...
        return e.reply(`『${standardMainName}』不存在，可能是未同步/无该角色？`);
      }
      ...
      for (let batchNum = 1; batchNum <= totalBatches; batchNum++) {
        let tempHtmlFilePath = "";
        let tempImgFilePath = "";
        const tempFileNameBase = `visualize-${standardMainName.replace(/[^a-zA-Z0-9]/g, "_")}-batch${batchNum}-${Date.now()}`;
        ...
        try {
          ...
          logger.info(`${logPrefix} [可视化] 正在生成第 ${batchNum}/${totalBatches} 批 (${currentBatchFiles.length} 张图片)...`);
  ...
  async ExportSingleImage(e) {
    ...
    try {
      ...
      try {
        ...
        this.logger.info(`${this.logPrefix} [导出] 成功读取文件到 Buffer: ${targetFileName}, 大小: ${fileBuffer.length} bytes`);
      } catch (readError) {
      ...
      try {
        if (sendErr?.message?.includes("highway") || sendErr?.message?.includes("file size") || sendErr?.code === -36 || sendErr?.code === 210005 || sendErr?.code === 210003) {
          await e.reply(`发送文件失败了,文件通道好像出了点问题 (${sendErr.code || "未知代码"})，可能是文件太大、网络不好或者被QQ限制了。`, true);
        } else {
  ...
  async TriggerError(e) {
    ...
    const match = e.msg.match(/#咕咕牛触发错误(?:\s*(git|fs|config|data|ref|type|Repo1|Repo2|notify|intermediate|other))?/i);
    ...
    try {
      switch (errorType) {
        ...
        case "Repo2":
          ...
          mockError.code = ERROR_CODES.NotFound;
          if (await MiaoPluginMBT.IsTuKuDownloaded(2)) await fsPromises.access(path.join(MiaoPluginMBT.paths.LocalTuKuPath2, "non-existent"));
    ...
    try {
      ...
      const processSpeeds = (speeds) => {
        return speeds
          ...
          .sort((a, b) => (a.priority ?? 999) - (b.priority ?? 999) || (a.speed === Infinity || a.statusText === "na" ? 1 : b.speed === Infinity || b.statusText === "na" ? -1 : a.speed - b.speed));
      };
      ...
      tempHtmlFilePath = path.join(MiaoPluginMBT.paths.tempHtmlPath, `manual-speedtest-temp-${Date.now()}-${Math.random().toString(16).slice(2)}.html`);
      try {
        await fsPromises.copyFile(sourceHtmlPath, tempHtmlFilePath);
      } catch (copyErr) {
  ...
  async handleSwitchCommand(e, configKey, featureName, enable) {
    ...
    if (configChanged) {
      ...
      try {
        ...
        panelSent = false;
        let finalUserMessage = `${logPrefix} ${featureName}已成功设为「${enable ? "开启" : "关闭"}」，但面板图片发送失败。`;
        if (saveWarning) finalUserMessage += `\n${saveWarning}`;
        await e.reply(finalUserMessage, true);
      }
    }
  ...
  static async SaveTuKuConfig(configData, logger = global.logger || console) {
    ...
    try {
      ...
      logger.error(`${logPrefix} [保存配置] 写入配置文件 ${configFilePath} 失败:`, error);
      return false;
    }
  }
      ...
      if (error.code === ERROR_CODES.NotFound) {
        try {
          ...
          if (srcError.code === ERROR_CODES.NotFound) {
            ...
            logger.error(`${Default_Config.logPrefix} [加载元数据] 加载或解析仓库源文件失败 (${secondaryPath}):`, srcError);
            data = null;
            success = false;
          }
        }
    ...
    if (Array.isArray(finalData)) {
      ...
      const validData = finalData
        .filter((item) => {
          ...
          if (!pathIsValid) logger.warn(`${Default_Config.logPrefix} [加载元数据] 过滤掉格式错误的图片路径: ${item.path}`);
          return pathIsValid;
        })
        .map((item) => ({ ...item, path: item.path.replace(/\\/g, "/") }));
      const validCount = validData.length;
      const invalidCount = originalCount - validCount;
      if (invalidCount > 0) logger.warn(`${Default_Config.logPrefix} [加载元数据] 在处理过程中忽略了 ${invalidCount} 条无效或格式错误的元数据。`);
      logger.info(`${Default_Config.logPrefix} [加载元数据] 处理完成，存在 ${validCount} 条有效元数据。`);
      return validData;
    } else {
  ...
  static async ScanLocalImagesToBuildCache(logger = global.logger || console) {
    ...
    logger.info(`${Default_Config.logPrefix} [扫描回退] 开始扫描本地仓库: ${ReposToScan.map((r) => r.name).join(", ")}...`);
            ...
            if (charDir.isDirectory()) {
              ...
              try {
                ...
                if (readCharErr.code !== ERROR_CODES.NotFound && readCharErr.code !== ERROR_CODES.Access)
                  logger.warn(`${Default_Config.logPrefix} [扫描回退] 读取角色目录 ${charFolderPath} 失败:`, readCharErr.code);
              }
            }
          }
        } catch (readGameErr) {
    ...
    logger.info(`${Default_Config.logPrefix} [扫描回退] 扫描完成，共找到 ${fallbackCache.length} 个独立的 .webp 图片文件。`);
  ...
  static async ReportError(e, operationName, error, context = "", logger = global.logger || console) {
    ...
    if (Report.stack) {
      ...
      const stackInfo = Report.stack.length > maxStackLength ? Report.stack.substring(0, maxStackLength) + "... (后面省略了)" : Report.stack;
      messagesToSend.push(`【🛠️ 技术细节 - 调用栈(部分)】\n${stackInfo}`);
    }
    try {
      const shortMessage = `${Default_Config.logPrefix} 执行 ${operationName} 操作时遇到点问题！(错误码: ${error?.code || "未知"})`;
      await e.reply(shortMessage, true);
      if (messagesToSend.length > 0 && common?.makeForwardMsg) {
        try {
          const forwardMsg = await common.makeForwardMsg(e, messagesToSend, `${Default_Config.logPrefix} ${operationName} 失败日志`);
          if (forwardMsg) {
            await e.reply(forwardMsg);
          } else {
          ...
          logger.warn(`${Default_Config.logPrefix} [错误报告] 创建/发送合并消息失败 (${forwardError.message})，尝试发送文本...`);
        ...
        logger.warn(`${Default_Config.logPrefix} [错误报告] 无法创建合并消息 (common.makeForwardMsg 不可用或消息为空)。`);
  ...
  static FormatError(operationName, error, context = "") {
    ...
    const Report = {
      summary: `${logPrefix} 操作 [${operationName}] 失败了！`,
      contextInfo: context || "（无额外上下文信息）", // 提供默认值
      suggestions: "",
      stack: error?.stack || "（调用栈信息丢失）", // 防止 error 为 null/undefined
    };
    ...
    // 构建一个包含所有可能错误信息的字符串，用于匹配 suggestionsMap
    const errorStringForSuggestions = `${error?.message || ""} ${stderr} ${String(error?.code) || ""} ${context || ""}`.toLowerCase();
    ...
    if (error instanceof ReferenceError && error.message?.includes("is not defined")) {
      ...
      for (const keyword in suggestionsMap) {
        // 对于 ERROR_CODES.XXX 这种直接是值的，需要确保 keyword 是字符串形式的错误码
        const keywordToTest = ERROR_CODES[keyword] || keyword; // 如果 keyword 是 ERROR_CODES 的键名，则取其值
        const escapedKeyword = String(keywordToTest).replace(/[.*+?^${}()|[\]\\]/g, "\\$&"); // 确保是字符串再转义
    ...
    finalSuggestionsArray.push(
      "- 请检查您的网络连接是否通畅。",
      "- 请检查 Yunzai-Bot 目录及插件相关目录的文件/文件夹权限。",
      "- 请仔细查看控制台输出的详细错误日志，特别是本条错误上下的内容。",
      "- 尝试重启 Yunzai-Bot 程序。"
    );
    if (operationName.toLowerCase().includes("下载") || operationName.toLowerCase().includes("更新")) {
      finalSuggestionsArray.push("- 确保您的设备上已正确安装 Git，并且 Git 的路径已添加到系统环境变量中。", "- 尝试执行 `#咕咕牛测速` 命令检查网络节点状况。");
    }
    if (!operationName.toLowerCase().includes("重置")) {
      // 如果不是重置操作本身失败
      finalSuggestionsArray.push("- 如果问题持续存在，作为最终手段，您可以尝试执行 `#重置咕咕牛` 命令，然后重新 `#下载咕咕牛` (请注意：这将清除所有咕咕牛图库相关数据和配置)。");
    }

    Report.suggestions = finalSuggestionsArray.join("\n");

    // --- stdout 和 stderr 到 contextInfo ---
    if (stdout || stderr) {
      Report.contextInfo += "\n\n--- Git/命令输出信息 ---"; // 加个换行与之前的 context 分隔
      const maxLen = 700; // 输出信息的最大长度
      if (stdout.trim()) {
        // 只在 stdout 有实质内容时显示
      ...
      if (stderr.trim()) {
        ...
        // 提取关键 stderr 行用于摘要，附加到 Report.summary
        const criticalStderrLines = stderr
          .split("\n")
          .filter(
            (line) =>
              ...
              !/^\s*$/.test(line) &&
              !/trace:|http\.c:|ssl\.c:|git\.c:|run-command\.c:|credential\.c:|config\.c:|advice\.c:|pktline\.c:|pack\.c:|sha1_file\.c:|remote\.c:|connect\.c:|version\.c:|sequencer\.c:|refs\.c:|commit\.c:|diff\.c:|unpack-trees\.c:|resolve-undo\.c:|notes-utils\.c:/i.test(
                line
              ) && // 过滤更多Git内部模块名
              !/^\s*(?:default|hint|Performance)\s/i.test(line) && // 过滤Git的advice和performance hint
              !/== Info:|\s*Trying\s|\s*Connected to\s|Receiving objects:|Resolving deltas:|remote: Compressing objects:|remote: Total|remote: Enumerating objects:|remote: Counting objects:/i.test(
                line
              )
          )
          .map((line) => line.replace(/^remote:\s*/, "").trim()) // 去掉 "remote: " 前缀并trim
          .filter((line) => line.length > 0) // 再次确保非空
          .slice(0, 5) // 取前5条
          .join("\n");
  ...
  static async LoadUserBans(forceReload = false, logger = global.logger || console) {
    ...
    if (success && Array.isArray(data)) {
      ...
      if (invalidOrDuplicateCount > 0) logger.warn(`${Default_Config.logPrefix} [加载用户封禁] 忽略 ${invalidOrDuplicateCount} 条无效/重复。`);
  ...
  static async SaveUserBans(logger = global.logger || console) {
    ...
    try {
      ...
      logger.error(`${logPrefix} [保存用户封禁] 写入配置文件 ${banListPath} 或其他操作失败!`, error);
      return false;
    }
  }
  /**
      ...
      if (targetPath) {
        deletePromises.push(
          fsPromises
            ...
            .catch((unlinkErr) => {
              if (unlinkErr.code !== ERROR_CODES.NotFound) logger.warn(`${Default_Config.logPrefix} [应用封禁] 删除 ${targetPath} 失败:`, unlinkErr.code);
            })
        );
      }
    }
  ...
  static async DetermineTargetPath(relativePath) {
    ...
    for (const fileSync of MiaoPluginMBT.paths.filesToSyncToCommonRes) {
      if (normalizedRelativePath === fileSync.sourceSubPath.replace(/\\/g, "/")) return path.join(MiaoPluginMBT.paths.commonResPath, fileSync.destFileName);
    }
    ...
    for (const repo of reposToSearch) {
      ...
      if (repoExists) {
        ...
        try {
          ...
          if (err.code !== ERROR_CODES.NotFound) {
            logger.warn(`${Default_Config.logPrefix} [查找路径] 访问仓库 ${repo.name} (${absPath}) 出错:`, err.code);
          }
        }
      }
    }
    // 如果所有仓库都没找到
    logger.warn(`${Default_Config.logPrefix} [查找路径] 在所有已下载仓库中均未找到: ${normalizedPath}`);
    return null;
  }
  ...
  static async CheckRoleDirExists(roleName) {
    ...
    const gameFolders = Object.values(MiaoPluginMBT.paths.sourceFolders).filter((f) => f !== MiaoPluginMBT.paths.sourceFolders.gallery);
    const ReposToCheck = [];
  ...
  static async DownloadRepoWithFallback(repoNum, repoUrl, branch, finalLocalPath, eForProgress, loggerInstance = global.logger || console) {
    ...
    const repoTypeName = repoNum === 1 ? "核心仓库" : `附属仓库(${repoNum}号)`;

    const timestamp = Date.now();
    const outerRandomSuffix = crypto.randomBytes(4).toString("hex");

    const uniqueTempCloneDirName = `GuTempClone-${repoNum}-${timestamp}-${outerRandomSuffix}`;
    const tempRepoPath = path.join(MiaoPluginMBT.paths.tempPath, "guguniu-downloads", uniqueTempCloneDirName);

    //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] 目标: ${finalLocalPath}, 初始临时克隆路径: ${tempRepoPath}`); //调试日志-精简
    ...
    try {
      ...
      //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 网络测速阶段失败:`, testError); //调试日志-精简
      if (eForProgress) {
        await MiaoPluginMBT.ReportError(eForProgress, `网络测速失败`, testError, `无法为 ${repoTypeName} 开始下载`, loggerInstance);
      }
    ...
    if (repoNum === 1 && eForProgress) {
      ...
      let tempImgFilePath_InitialSpeedtest = "";
      let screenshotNameInitial = `guguniu-initial-dl-speedtest-${outerRandomSuffix}`;
      ...
      try {
        if (allTestResults.length > 0) {
          ...
          try {
            ...
            const processSpeeds = (speeds) =>
              speeds
                .map((s) => ({ ...s, statusText: s.testUrlPrefix === null ? "na" : Number.isFinite(s.speed) && s.speed >= 0 ? "ok" : "timeout" }))
                .sort(
                  (a, b) => (a.priority ?? 999) - (b.priority ?? 999) || (a.speed === Infinity || a.statusText === "na" ? 1 : b.speed === Infinity || b.statusText === "na" ? -1 : a.speed - b.speed)
                );
            ...
            if (bestSourceForReport_Initial) {
              let speedInfo = "N/A";
              if (bestSourceForReport_Initial.testUrlPrefix !== null)
                speedInfo = Number.isFinite(bestSourceForReport_Initial.speed) && bestSourceForReport_Initial.speed >= 0 ? `${bestSourceForReport_Initial.speed}ms` : "超时";
            ...
            tempHtmlFilePath_InitialSpeedtest = path.join(MiaoPluginMBT.paths.tempHtmlPath, `initial-dl-speedtest-rendered-${timestamp}-${outerRandomSuffix}.html`);
            await fsPromises.writeFile(tempHtmlFilePath_InitialSpeedtest, htmlContent_Initial, "utf8");
          } catch (prepOrRenderError) {
            //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 准备或渲染初始测速报告失败:`, prepOrRenderError); //调试日志-精简
            if (eForProgress) await eForProgress.reply(`${logPrefix} 准备或渲染初始测速报告出错，将继续下载...`).catch(() => {});
          ...
          if (canGenerateInitialReport) {
            try {
              await fsPromises.mkdir(MiaoPluginMBT.paths.tempImgPath, { recursive: true });
              tempImgFilePath_InitialSpeedtest = path.join(MiaoPluginMBT.paths.tempImgPath, `initial-dl-speedtest-img-${timestamp}-${outerRandomSuffix}.png`);
              const img_Initial = await puppeteer.screenshot(screenshotNameInitial, {
                tplFile: tempHtmlFilePath_InitialSpeedtest,
                savePath: tempImgFilePath_InitialSpeedtest,
                imgType: "png",
                pageGotoParams: { waitUntil: "networkidle0" },
                ...renderData_Initial,
                screenshotOptions: { fullPage: false },
              ...
              if (img_Initial) {
                if (eForProgress) {
                  await eForProgress.reply(img_Initial).catch(() => {});
                  await common.sleep(500);
                  //await eForProgress.reply(`${logPrefix} 初始测速完成，开始下载 ${repoTypeName}...`).catch(()=>{}); //调试日志-精简
                }
              } else {
                if (eForProgress) await eForProgress.reply(`${logPrefix} 生成初始测速报告图片为空，继续下载...`).catch(() => {});
              }
            } catch (screenshotError) {
              //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] Puppeteer 生成初始测速截图失败:`, screenshotError); //调试日志-精简
              if (eForProgress) await eForProgress.reply(`${logPrefix} 生成初始测速报告截图出错，继续下载...`).catch(() => {});
            } finally {
              if (tempHtmlFilePath_InitialSpeedtest && fs.existsSync(tempHtmlFilePath_InitialSpeedtest)) {
                try {
                  await fsPromises.unlink(tempHtmlFilePath_InitialSpeedtest);
                } catch (unlinkErr) {}
              }
              if (tempImgFilePath_InitialSpeedtest && fs.existsSync(tempImgFilePath_InitialSpeedtest)) {
                try {
                  await fsPromises.unlink(tempImgFilePath_InitialSpeedtest);
                } catch (unlinkErr) {}
              }
              const possiblePuppeteerTempDir_Initial = path.join(MiaoPluginMBT.paths.tempPath, "..", screenshotNameInitial);
              if (fs.existsSync(possiblePuppeteerTempDir_Initial)) {
                try {
                  await safeDelete(possiblePuppeteerTempDir_Initial);
                } catch (deleteErr) {}
              }
            }
          }
        } else {
          //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] 初始测速未返回有效代理结果，跳过报告生成。`); // 调试日志-精简
          if (eForProgress) await eForProgress.reply(`${logPrefix} 初始测速未找到可用节点，将直接尝试下载...`).catch(() => {});
        }
      } catch (outerInitialSpeedTestError) {
        //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 执行初始测速或排序源时出错(顶层):`, outerInitialSpeedTestError); //调试日志-精简
        if (eForProgress) await eForProgress.reply(`${logPrefix} 执行初始测速时发生错误，仍将尝试下载...`).catch(() => {});
        if (!sourcesToTry || sourcesToTry.length === 0) {
          if (allTestResults && allTestResults.length > 0) {
            sourcesToTry = MiaoPluginMBT.GetSortedAvailableSources(allTestResults, true, loggerInstance);
          } else {
            try {
              const fallbackTestResults = await MiaoPluginMBT.TestProxies(baseRawUrl, loggerInstance);
              sourcesToTry = MiaoPluginMBT.GetSortedAvailableSources(fallbackTestResults, true, loggerInstance);
            } catch (finalTestError) {
              loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 后备测速也失败:`, finalTestError); //调试日志-精简
    ...
    const tempWriteTestPath = path.join(MiaoPluginMBT.paths.tempPath, `write_test_${timestamp}.tmp`);
    try {
      await fsPromises.writeFile(tempWriteTestPath, "test");
      await fsPromises.unlink(tempWriteTestPath);
    } catch (writeError) {
      loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 无法写入临时目录 ${MiaoPluginMBT.paths.tempPath}. 请检查权限。`, writeError);
      if (eForProgress) await MiaoPluginMBT.ReportError(eForProgress, `下载${repoTypeName}`, new Error("无法写入临时目录"), `路径: ${MiaoPluginMBT.paths.tempPath}`, loggerInstance);
      return { success: false, nodeName: "环境检查失败", error: new Error("无法写入临时目录") };
    }

    if (!sourcesToTry || sourcesToTry.length === 0) {
      //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 在下载决策阶段无可用下载源！`); //调试日志-精简
      const error = new Error("无可用下载源 (所有测速均失败或无结果)");
      if (eForProgress) await MiaoPluginMBT.ReportError(eForProgress, `下载${repoTypeName}`, error, "所有测速尝试均未找到可用节点", loggerInstance);
      return { success: false, nodeName: "无可用源", error };
    }

    //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] 下载决策可用源: ${sourcesToTry.map(s => s.name).join(', ')}`); //调试日志-精简
    ...
    if (repoNum === 1 && githubSourceFromConfig && githubTestResultForDecision && githubTestResultForDecision.speed !== Infinity && githubTestResultForDecision.speed <= 300) {
      ...
      if (githubIndexInSources > 0) {
        const [githubItem] = sourcesToTry.splice(githubIndexInSources, 1);
        sourcesToTry.unshift(githubItem);
        //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] GitHub直连速度良好 (${githubTestResultForDecision.speed}ms)，调整为最优先尝试。`); //调试日志-精简
        ...
        //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] GitHub直连速度良好 (${githubTestResultForDecision.speed}ms)，强制添加到最优先尝试。`);//调试日志-精简
      }
    }
    ...
    for (const source of sourcesToTry) {
      ...
      if (source.name === "GitHub") {
        ...
        if (ghResult && (ghResult.speed === Infinity || ghResult.speed > 300)) {
          //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] GitHub直连速度 (${ghResult.speed === Infinity ? '超时' : ghResult.speed + 'ms'}) 不佳 (>300ms)，跳过此源。`); //调试日志-精简
          continue;
        }
      }
      ...
      if (!isFirstActualDownloadAttempt && eForProgress && repoNum === 1) {
        if (eForProgress) eForProgress.reply(`『咕咕牛』${repoTypeName} 尝试节点: ${nodeName}...`).catch(() => {});
      }
      ...
      for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        ...
        try {
          if (attempt > 1) {
            ...
            // loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] (${nodeName}) 第 ${attempt -1}/${maxAttempts -1} 次重试，等待 ${delay/1000}s...`); //调试日志-精简
            if (eForProgress && repoNum === 1) {
              eForProgress.reply(`『咕咕牛』${repoTypeName}(${nodeName}) 第 ${attempt - 1} 次下载失败，${delay / 1000}s 后重试 (总尝试 ${maxAttempts})...`).catch(() => {});
            }
          ...
          if (repoPathMatch && repoPathMatch[1]) {
            ...
            throw new Error(`(${nodeName}) 无法从原始repoUrl (${repoUrl}) 提取仓库路径。`);
          }
          ...
          if (source.name === "GitHub") {
            ...
            if (source.name === "GitClone") {
              actualCloneUrl = `${cleanCloneUrlPrefix}/${repoUrl.replace(/^https?:\/\//, "")}`;
            } else if (source.name === "Mirror" || source.cloneUrlPrefix.includes("gitmirror.com")) {
              actualCloneUrl = `${cleanCloneUrlPrefix}/${userAndRepoPath}`;
            } else {
              actualCloneUrl = `${cleanCloneUrlPrefix}/${repoUrl}`;
            }
            try {
              const proxyUrlObj = new URL(source.cloneUrlPrefix);
            } catch (e) {}
          } else {
            throw new Error(`(${nodeName}) 源配置缺少 cloneUrlPrefix。`);
          }

          const cloneArgs = ["clone", "--verbose", `--depth=${Default_Config.gitCloneDepth}`, "--progress", actualCloneUrl, tempRepoPath];
          const gitOptions = { cwd: MiaoPluginMBT.paths.YunzaiPath, shell: false };


          //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] (${nodeName}) 第 ${attempt}/${maxAttempts} 次尝试: git ${cloneArgs.join(" ")}`); //调试日志-精简
            ...
            await ExecuteCommand("git", cloneArgs, gitOptions, Default_Config.gitCloneTimeout, (stderrChunk) => {
              if (eForProgress && repoNum === 1) {
                ...
                if (match?.[1]) {
                  ...
                  if (percent >= 10 && !progressStatus.reported10) {
                    progressStatus.reported10 = true;
                    eForProgress.reply(`『咕咕牛』${repoTypeName}(${nodeName})下载:10%...`).catch(() => {});
                  }
                  if (percent >= 90 && !progressStatus.reported90) {
                    progressStatus.reported90 = true;
          ...
          //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] (${nodeName}) 克隆到 ${tempRepoPath} 成功，开始部署...`); //调试日志-精简

          const intermediateDirName = `intermediate-${repoNum}-${timestamp}-${outerRandomSuffix}-${source.name.replace(/[^a-zA-Z0-9]/g, "")}-${attempt}`;
          currentIntermediateDir = path.join(path.dirname(tempRepoPath), intermediateDirName);
          ...
          try {
            await fsPromises.rename(tempRepoPath, intermediatePathForAttempt);
          } catch (moveError) {
            if (moveError.code === "EPERM" || moveError.code === "EXDEV") {
              await copyFolderRecursive(tempRepoPath, intermediatePathForAttempt, {}, loggerInstance);
              await safeDelete(tempRepoPath);
            } else {
              throw moveError;
            }
          ...
          try {
            await fsPromises.rename(intermediatePathForAttempt, finalLocalPath);
          } catch (finalMoveError) {
            if (finalMoveError.code === "EPERM" || finalMoveError.code === "EXDEV") {
              await copyFolderRecursive(intermediatePathForAttempt, finalLocalPath, {}, loggerInstance);
          ...
          //loggerInstance.info(`${logPrefix} [下载流程 ${repoTypeName}] (${nodeName}) 成功部署到 ${finalLocalPath}`); //调试日志-精简
          return { success: true, nodeName, error: null };
        } catch (error) {
          lastError = error;
          //loggerInstance.warn(`${logPrefix} [下载流程 ${repoTypeName}] (${nodeName}) 第 ${attempt}/${maxAttempts} 次尝试失败: ${error.message}`); //调试日志-精简
          //if (error.stderr) loggerInstance.debug(`${logPrefix} Stderr for ${nodeName} attempt ${attempt}: ${error.stderr}`); //调试日志-精简

          await safeDelete(tempRepoPath);
          if (currentIntermediateDir) {
            await safeDelete(currentIntermediateDir);
            currentIntermediateDir = null;
          }
          ...
          if (stderrText.includes("repository not found") || stderrText.includes("authentication failed") || (error.code === 128 && stderrText.includes("access rights"))) {
            //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] (${nodeName}) 遇到不可恢复错误，不再对此源重试。`); //调试日志-精简
            break;
          }
        }
      }
    ...
    //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 所有下载源及重试均失败。`); //调试日志-精简
    const finalErrorToReport = lastError || new Error("所有下载尝试均失败，无具体错误信息");
    if (eForProgress && repoNum === 1) {
      await MiaoPluginMBT.ReportError(eForProgress, `下载${repoTypeName}`, finalErrorToReport, "已尝试所有可用下载源及其重试次数。", loggerInstance);
      ...
      //loggerInstance.error(`${logPrefix} [下载流程 ${repoTypeName}] 最终失败: Node: ${finalErrorToReport?.nodeName || 'N/A'}`, errorDetail); //调试日志-精简
    }
  ...
  static async UpdateSingleRepo(
    ...
    try {
      ...
      if (success) {
        ...
        // 记录日志信息
        if (hasChanges) {
          logger.info(`${Default_Config.logPrefix} [更新仓库] ${RepoName} 检测到新的提交${wasForceReset ? " (通过强制重置)" : ""}。`);
          ...
          logger.info(`${Default_Config.logPrefix} [更新仓库] ${RepoName} pull 操作完成，未检测到新提交或无 "Already up to date" 消息。`);
  ...
  static async RunPostUpdateSetup(e, isScheduled = false, logger = global.logger || console) {
    try {
      ...
      if (!isScheduled && e) await MiaoPluginMBT.ReportError(e, "更新后设置", error, "", logger);
        ...
        logger.error(`${Default_Config.logPrefix}--- 定时更新后设置失败 ----\n${Report.summary}\n${Report.suggestions}\n---`);
      }
    }
  }
          ...
          (async () => {
            try {
              ...
              try {
                ...
                errorCount++;
                if (copyErr.code !== ERROR_CODES.NotFound) logger.warn(`${Default_Config.logPrefix} [同步角色] 复制失败: ${path.basename(sourcePath)} -> ${targetPath}`, copyErr.code);
              }
      ...
      for (const entry of entries) {
        ...
        if (entry.isDirectory()) {
          ...
          try {
            ...
            for (const fileToDelete of filesToDelete) {
              ...
              try {
                ...
                if (unlinkErr.code !== ERROR_CODES.NotFound) logger.warn(`${Default_Config.logPrefix} [清理目标] 删除文件 ${filePath} 失败:`, unlinkErr.code);
              }
            }
          } catch (readSubErr) {
            if (![ERROR_CODES.NotFound, ERROR_CODES.Access].includes(readSubErr.code)) logger.warn(`${Default_Config.logPrefix} [清理目标] 读取角色子目录 ${characterPath} 失败:`, readSubErr.code);
          }
          ...
          try {
            ...
            if (delErr.code !== ERROR_CODES.NotFound) logger.warn(`${Default_Config.logPrefix} [清理目标] 删除根目录文件 ${rootFilePath} 失败:`, delErr.code);
          }
        }
      }
      ...
      try {
        if (proxy.name === "GitHub") {
          ...
          try {
            ...
            logger.warn(`${Default_Config.logPrefix} [网络测速] 构造的代理URL (${testUrl}) 格式可能不规范:`, urlError.message);
          }
        } else {
          return {
            name: proxyName,
