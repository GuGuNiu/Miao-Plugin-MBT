...
&emsp;&emsp;图库管理器是咕咕牛图库的专用Js，为提升下载成功率使用多个镜像站自动测速最优下载以及JSON作为索引数据源`(PM18面板图则不受JSON索引相关功能的约束)`，请不要手动删除图库下载的任何文件当前是全静态加载，否则将无法通过Js初始化自检，十分不建议手动修改管理器内任何代码，当前代码存在高耦合，有疑问可以喵群AT咨询，issues处理不及时。
