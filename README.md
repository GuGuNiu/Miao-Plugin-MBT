...
&emsp;&emsp;图库管理器是咕咕牛图库的专用Js，遇到本地冲突将会自动强制重置，操作携带有结构化日志以及问题建议，使用多个镜像站自动测速最优下载以及JSON作为索引数据源(PM18特殊面板图则不受任何功能的约束)，请不要手动删除图库下载的任何文件当前是全静态加载，否则将无法通过Js初始化自检。

🔶2025年起将提供超高清以上面板图，不再使用巨型水印，新模板已使用QR Code代替
