...
function filterImages(query) {
    ...
    console.log('正在过滤:', lowerQuery, '于', _availableImages.length, '个项目中');
    ...
    console.log('过滤数量 (检查存在前):', filtered.length);
    const finalFiltered = filtered.filter(img => {
        if (!img.urlPath) { 
             console.warn('过滤时发现图片缺少 urlPath:', img);
             return false;
        }
        const exists = _existingImagePaths.has(img.urlPath); 
        if (exists) {
             console.log(`filterImages: Path found in existingPaths, filtering out: ${img.urlPath}`);
        } else {
             console.log(`filterImages: Path NOT found in existingPaths, keeping: ${img.urlPath}`);
        }
        return !exists;
    });

    console.log('最终过滤数量:', finalFiltered.length);
...
function findSiblings(currentImageInfo) {
    if (!currentImageInfo || !currentImageInfo.gallery || !currentImageInfo.folderName || !_availableImages || _availableImages.length === 0) {
         console.warn('findSiblings 参数不足:', currentImageInfo);
        return [];
    }
    console.log('正在查找同级图片:', currentImageInfo.folderName, '在图库:', currentImageInfo.gallery);
    ...
    console.log('找到同级图片数量 (检查存在前):', siblings.length);
    const finalSiblings = siblings.filter(img => {
        if (!img.urlPath) return false;
        const exists = _existingImagePaths.has(img.urlPath);
        return !exists;
    });
    console.log('最终可显示同级图片数量:', finalSiblings.length);
...
self.onmessage = function(event) {
    ...
    try {
        if (type === 'loadData') {
            ...
            console.log(`已加载 ${_availableImages.length} 张图片和 ${_existingImagePaths.size} 个已存在路径 (相对路径)`);
            ...
            console.log('已发送', results.length, '个同级结果');
        }
    } catch (error) {
...
console.log('Worker 脚本已加载并准备就绪 (旧版逻辑)');